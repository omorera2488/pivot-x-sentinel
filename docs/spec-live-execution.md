...
## 12. Kill switch de pérdida diaria (BOT-032)

**Objetivo:** protección del motor de ejecución (no de la estrategia): si la pérdida NETA realizada del día operativo alcanza un límite configurado, el bot deja de colocar operaciones **nuevas**. Nunca cierra posiciones abiertas ni cancela pendientes por sí solo, y nunca detiene el `run()` — ver por qué en los dos puntos siguientes.

- **El bucle `run()` NUNCA se detiene por esto.** El kill switch bloquea únicamente la llamada a `_place_order()` dentro de `process_closed_bar()` — todo lo demás (§6 reconciliación, §7 vigilancia de pendientes, §8 cierre por tiempo máximo, SL/TP nativos del bróker) sigue funcionando exactamente igual, disparado o no el kill switch. Detener el thread perdería esa gestión de las posiciones que ya están abiertas (decisión explícita, no accidental — una posición sin `_watch_open()` corriendo pierde el cierre por tiempo máximo, quedando solo con el SL/TP nativo).
- **Punto exacto del guard:** en `LiveExecutionBot.process_closed_bar()`, junto al chequeo de concurrencia existente (`activos >= limite`) — si `_kill_switch_blocking()` es verdadero, se loguea "Señal descartada: kill switch de perdida diaria activo" y no se llama a `_score_entry()`/`_place_order()`. El estado que usa ese chequeo (`_kill_switch_triggered`/`_kill_switch_override_active`) se recalcula en `_refresh_kill_switch_state()`, llamado en `run()` **antes** de `poll_once()` en cada ciclo (dentro del mismo bloque `with mt5_lock:`) — así cualquier orden que se vaya a colocar en ese ciclo usa el P&L más fresco posible antes de tocar el bróker.
- **Latencia residual (documentada a propósito):** una pérdida que se realiza DENTRO del mismo ciclo (ej. un cierre por tiempo en `_watch_open`, que corre después del refresh) recién se refleja en el ciclo siguiente — como máximo `poll_interval_s` (10s típico), mismo orden de magnitud que la diferencia ya existente entre §7 (`_watch_pending`, por vela) y su contraparte por tick en vivo.
- **Día operativo:** `execution/src/operating_day.py` define el día como 00:00:00 → 00:00:00 (día siguiente) hora LOCAL de una timezone IANA (`DEFAULT_OPERATING_TIMEZONE = "America/Costa_Rica"`, constante de módulo — nunca "UTC menos N horas" a mano, se delega a `zoneinfo` para que un cambio de horario de verano no rompa nada). Es el backend, nunca el navegador/PC del cliente, quien decide esta frontera — el panel (`panel/calendar.html`) la consume vía `GET /status`'s `operating_timezone`.
- **Cálculo del P&L:** `execution/src/kill_switch.py::daily_realized_pnl()` reusa la misma reconciliación segura de deals que `GET /history` (`mt5_utils.filter_own_deals()`, ver `docs/spec-api.md`) para no reintroducir el bug de "deal de cierre manual sin magic propio". Solo cuenta deals de CIERRE (`DEAL_ENTRY_OUT`) ya realizados dentro del día operativo — nunca P&L flotante de posiciones abiertas.
- **Fail-safe:** si `mt5.history_deals_get()` no devuelve datos, el kill switch se trata como DISPARADO (nunca se asume P&L = $0) — bloquea nuevas entradas hasta que MT5 vuelva a responder o el usuario confirme el override explícitamente.
- **Override diario:** persiste en `execution/src/kill_switch_store.py` (`user_data_root()/execution/data/kill_switch/`, mismo mecanismo que sobrevive upgrades del instalador que `execution/src/score_store.py`) como `{symbol}_{magic} -> fecha operativa del último override confirmado`. Su validez se deriva comparando esa fecha contra la fecha operativa ACTUAL en cada evaluación — un día operativo nuevo invalida el override anterior sin depender de ningún timer. El P&L en sí nunca se persiste: se recalcula siempre desde MT5.
- **Arranque con el límite ya alcanzado:** el chequeo vive en `api/app.py::_kill_switch_gate()`, llamado dentro de `POST /start` ANTES de crear el `LiveExecutionBot` — un reinicio de la app no puede evadir la protección, porque el P&L se recalcula desde el historial real de MT5 en ese mismo momento, no desde un contador en memoria que se perdió al reiniciar.
