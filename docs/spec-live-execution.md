...
Esto reemplaza a los arrays `pending`/`open` en memoria que usa el motor de backtest (`spec-backtest.md`, `src/engine.py`) — en vivo, la lista de pendientes/abiertas ES la que devuelve el bróker, no una que el proceso mantenga por su cuenta. El **conteo de concurrencia** (`spec-estrategia.md` §6) se calcula sobre estas listas reales, no sobre una copia en memoria que podría desincronizarse si una orden se llenó, canceló o cerró por fuera del bot (ej. el usuario la tocó manualmente, o el bróker la cerró por stop-out).
