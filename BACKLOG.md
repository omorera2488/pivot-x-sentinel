...
- **Subtareas/experimentos:**
  ...
  - `BOT-047` `IN PROGRESS / RESEARCH` — D1 Alignment (feature padre, ver "Convención para features experimentales" más abajo):
    ...
    - `BOT-047.3` `BLOCKED` — D1 Alignment Out-of-Sample Validation (espera revisión humana de los freezes `PROVISIONAL` de `BOT-047.2` y `BOT-047.2.1`).
...
- **Subtareas:**
  ...
  - `BOT-047.3` `BLOCKED` — OOS Validation (espera revisión humana explícita de los freezes `PROVISIONAL` de `BOT-047.2` y `BOT-047.2.1`, no se desbloquea automáticamente).
...
- **Siguiente paso planificado dentro de BOT-047 (bloqueado, ver `BOT-047.3`):** revisión humana explícita que decida específicamente qué hacer con el boundary no documentado del legacy (usar tal cual / corregir a 22:00 UTC / investigar más) antes de desbloquear `BOT-047.3`.
