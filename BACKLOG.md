...
- **Subtareas/experimentos:**
  ...
  - `BOT-051` `IN PROGRESS / RESEARCH` — Signal Quality Integration (feature padre; adopta subestructura `.1/.2/.3/.4/.5/.6` — ver esa entrada):
    ...
    - `BOT-051.6` `TODO — NEXT ACTIVE` — Signal Quality OOS Readiness Review.

  BOT-024 en sí (la definición formal de Signal Quality 0–100) sigue `TODO` — no se cierra hasta tener evidencia suficientemente validada (OOS) de al menos Momentum y Alignment. Su continuación pasa ahora por `BOT-051.6`, próximo trabajo activo del proyecto tras el cierre de `BOT-051.5`.
...
- **Siguiente paso planificado dentro de BOT-051 (ver `BOT-051.6`):** Signal Quality OOS Readiness Review — cuando la acumulación real alcance cobertura suficiente en algún factor, revisar manualmente si corresponde avanzar hacia `BOT-024.3`/`BOT-047.3`/`BOT-048.3`/`BOT-050.3`. La acumulación sigue en paralelo al desarrollo; la falta temporal de N suficiente no bloquea otras tareas de ingeniería/observabilidad/documentación.
...
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.5`. Cuando `execution/src/signal_quality_oos_dataset.py::readiness()` reporte `READY_FOR_OOS_ANALYSIS` para algún factor (criterios ya codificados y documentados en `BOT-051.5` — cobertura direccional/de estados/de días, nunca "N≥50"), revisar manualmente si corresponde avanzar la validación OOS correspondiente (`BOT-024.3`/`BOT-047.3`/`BOT-048.3`/`BOT-050.3`) usando el dataset ya construido por el bridge. **No** reabre ninguna definición congelada, **no** reoptimiza, **no** hace feature fishing (misma disciplina que el resto de la línea experimental). Mientras tanto, la acumulación sigue en paralelo — este ticket no bloquea ni se bloquea por otro trabajo del proyecto.
