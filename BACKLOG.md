...
- **Subtareas:**
  ...
  - `BOT-048.2` `TODO` — Definition Freeze (no iniciada).
  - `BOT-048.3` `TODO` — OOS Validation (no iniciada, bloqueada por falta de histórico genuinamente posterior al 2026-09-15, mismo motivo que `BOT-024.3`/`BOT-047.3`).
...
- **Siguiente paso planificado dentro de BOT-048 (no ejecutado):** revisión humana de estos candidatos y freeze de una definición — `BOT-048.2`, en particular decidir si `origin_retracement_frac`/`origin_dist_atr` se tratan como Structure o si alguna se reclasifica como Economics (mismo tipo de decisión que `BOT-047.2.1`/`.2.2`/`.2.3` resolvieron para Alignment vs Regime).
