...
- **Subtareas/experimentos:**
  ...
  - `BOT-051` `IN PROGRESS / RESEARCH` — Signal Quality Integration (feature padre; adopta subestructura `.1/.2/.3/.4/.5` — ver esa entrada):
    ...
    - `BOT-051.5` `TODO — NEXT ACTIVE` — Signal Quality Live Accumulation / OOS Bridge.

  BOT-024 en sí (la definición formal de Signal Quality 0–100) sigue `TODO` — no se cierra hasta tener evidencia suficientemente validada (OOS) de al menos Momentum y Alignment. Su continuación pasa ahora por `BOT-051.5`, próximo trabajo activo del proyecto tras el cierre de `BOT-051.4`.
...
- **Categoría:** Scoring / Investigación / Definición formal
- **Estado:** IN PROGRESS / RESEARCH — `BOT-051.1` `DONE` (decisión `VECTOR_FIRST`), `BOT-051.2` `DONE` (decisión `SIGNAL_QUALITY_VECTOR_V1_FREEZE`), `BOT-051.3` `DONE` (decisión `SHADOW_OBSERVABILITY_READY`), `BOT-051.4` `DONE` (decisión `SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`), `BOT-051.5` `TODO — NEXT ACTIVE`
- **Prioridad:** HIGH
- **Incorporado:** 2026-09-20
...
- **Prioridad:** LOW (no bloquea nada — depende de tiempo/volumen real, no de trabajo pendiente)
- **Incorporado:** 2026-09-21
- **Versión objetivo:** a definir
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.4`. Con Signal Quality ya viva en producción (`SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`), no queda research nuevo que hacer hasta que el bot en vivo acumule suficientes snapshots reales junto a resultados reales de trades. Objetivo futuro: construir el puente hacia OOS genuino (`BOT-024.3`/`BOT-047.3`/`BOT-048.3`/`BOT-050.3`) usando exclusivamente datos que nunca participaron en ningún discovery — sin reabrir ninguna definición congelada, sin reoptimizar, sin feature fishing (misma disciplina que el resto de la línea `BOT-047.3`/etc.). **No** inicia investigación de score 0–100 ni gate — sigue fuera de alcance hasta validación OOS suficiente.
- **Dependencias:** BOT-051.4 (`DONE — SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`), histórico genuinamente posterior al 2026-09-15 con volumen suficiente (mismo bloqueo que `BOT-024.3`/`BOT-047.3`/`BOT-048.3`/`BOT-050.3`).
...
- **F. Signal Quality Integration (`BOT-051`) no exige cinco representaciones numéricas obligatorias — actualizado 2026-09-21 tras `BOT-051.4`:** tabla de estado actual — Momentum (`BOT-024.2` `DONE`, `BOT-024.4` `FREEZE_READY` — canónica `roc_atr_3`, RAW; OOS `BOT-024.3` `BLOCKED`), Alignment (`BOT-047.2.3` `FREEZE_READY`, OOS `READY/WAITING`), Structure (`BOT-048.2` `ORIGIN_ONLY_FREEZE`, OOS `PENDING/WAITING`), Economics (`BOT-049.2` `NO_VALID_ECONOMICS_FREEZE` — **excluida del vector, `EXCLUDED_ABSORBED`, no pendiente**), Context (`BOT-050.2` `TEMPORAL_ONLY_FREEZE` — **representación parcial**: solo `weekday`/Viernes, OOS `BLOCKED_WAITING_GENUINE_NEW_DATA`). `BOT-051.2` congeló el esquema `SignalQualityVectorV1` (`SIGNAL_QUALITY_VECTOR_V1_FREEZE`), `BOT-051.3` lo hizo observable en shadow sobre el 100% de Universo A (`SHADOW_OBSERVABILITY_READY`), y **`BOT-051.4` lo llevó a producción** (decisión `SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`: calculado/persistido/expuesto por API/panel para cada LIMIT real, parity exhaustiva 0 discrepancias contra el histórico N=3.207, behavior invariance probada A/B con MT5 falso — el request a `order_send()` es byte-idéntico con o sin Signal Quality, `strategy/engine.py`/`live_signal.py` con diff cero) — vector de 4 slots nominales (no 5), los 4 con Definition Freeze propio, ninguno OOS-validado todavía, **puramente observacional, sin participar en ninguna decisión de trading**. La integración futura debe usar únicamente los factores que sobrevivan sus propios contratos/validaciones — sin pesos, score 0–100, `confidence` ni gate definidos todavía (ver `BOT-051`, `BOT-051.1`–`.5`).
