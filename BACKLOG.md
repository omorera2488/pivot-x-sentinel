...
- **Sub-ítems:** `BOT-052.1` (Historical Discovery, `DONE`), `BOT-052.2` (HCH Definition / OOS Shadow Validation, `TODO`).
...
### BOT-052.2 — HCH Definition / OOS Shadow Validation
- **Categoría:** Scoring / Investigación / Validación
- **Estado:** TODO
- **Prioridad:** LOW
- **Incorporado:** 2026-09-24
- **Versión objetivo:** a definir
- **Descripción:** Subtarea de `BOT-052`, alcance determinado por la evidencia de `BOT-052.1` (solo HCH). Congelar la definición exacta de HCH documentada en `BOT-052.1` §3.4 (mapeo de inputs, `mintick`, consumo, tolerancia vacía), registrarla como observación **shadow** por LIMIT en vivo (sin participar de ninguna decisión) y evaluarla recién con histórico genuinamente posterior al 2026-09-15, con criterio pre-registrado antes de mirar resultados (p. ej. Δ ExpR de HCH > 0 fuera de muestra con N suficiente para un IC informativo). **Fuera de alcance:** gate, peso, score, cambio de entradas; `D`/`D+HCH`/EMA (no justificados por `BOT-052.1`). No iniciada.
- **Dependencias:** BOT-052.1 (`DONE`), datos OOS genuinos (misma barrera que BOT-024.3/BOT-047.3/BOT-048.3).
