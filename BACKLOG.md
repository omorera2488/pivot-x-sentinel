...
- **Incorporado:** 2026-09-21
- **Versión objetivo:** N/A — investigación offline, sin release de producción
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.1`. Debe formalizar el esquema tipado del vector `VECTOR_FIRST` decidido en `BOT-051.1` (`reports/BOT-051.1-signal-quality-integration-design.md` sección 15): estructura con campos `momentum`, `alignment` (enum 6 estados), `structure` (float continuo), `context` (enum 7 días), más `direction` como metadato obligatorio adjunto. **Actualización 2026-09-21 (`BOT-024.4`):** `momentum` ya NO es `PENDING_DEFINITION` — `BOT-024.4` congeló su contrato (`roc_atr_3`, RAW continua, `FREEZE_READY`) y `BOT-051.2` puede poblar ese slot con él. **NO** debe avanzar a Candidate B/C/D (composite state / ordinal / score 0-100) hasta que exista al menos una validación OOS genuina de alguno de los cuatro contratos (`BOT-024.3`/`BOT-047.3`/`BOT-048.3`/`BOT-050.3` — las cuatro siguen `BLOCKED`/`WAITING`).
- **Dependencias:** BOT-051.1 (`DONE`, decisión `VECTOR_FIRST` y semántica de missing de dos niveles a respetar), BOT-024.4 (`DONE — FREEZE_READY`, contrato de Momentum a poblar en el vector).
