...
- **Descripción:** Integrar las dimensiones de `Signal Quality` (`Momentum` + `Alignment` + `Structure` + `Economics` + `Context`) preservando significado, trazabilidad e interpretabilidad de cada una — **no asume weighted scoring ni pesos iguales/arbitrarios**. Dirección futura de producto a mantener: `SIGNAL QUALITY: XX/100` con tabla/desglose visible de los factores y, si todos cumplen causalidad, disponible desde `limit_created_bar`. La transformación 0–100, los pesos, el manejo de confianza/`confidence` y las reglas de combinación deben definirse **posteriormente, con evidencia** — no en esta entrada ni de forma automática al completarse `BOT-049`/`BOT-050`. `BOT-051` **no introduce por sí mismo ningún gate en vivo**. Desde `BOT-051.4` (2026-09-21), Signal Quality **existe y se ve dentro del bot productivo** (observacional, sin participar en decisiones de trading) — ver esa entrada.
  ...
  - **Estrategia de acumulación OOS (2026-09-22):** la acumulación OOS ocurre en paralelo al desarrollo normal del bot. El bajo número de operaciones reales por día y los períodos en que el bot permanece apagado pueden hacer que alcanzar cobertura suficiente tome tiempo; esto **no bloquea** trabajo de ingeniería, UI, observabilidad, documentación o preparación que no dependa estadísticamente del OOS — por eso `BOT-051.6` (Production UI) es el próximo trabajo activo mientras `BOT-051.7` (antes numerada `BOT-051.6`, OOS Readiness Review) sigue en espera. No se estiman fechas artificiales ni se relajan los criterios de readiness ya codificados en `BOT-051.5` para acelerarla.
