...
- **Subtareas/experimentos:**
  ...
  - `BOT-050` `TODO` — Context (feature padre, ver "Convención para features experimentales" más abajo):
    - `BOT-050.1` `TODO — NEXT ACTIVE` — Context Feature Discovery.
    - `BOT-050.2` `TODO` — Context Definition Freeze.
    - `BOT-050.3` `TODO — PENDING / WAITING GENUINE OOS` — Context OOS Validation.
  - `BOT-051` `TODO` — Signal Quality Integration (ID principal, no lleva subestructura `.1/.2/.3` — ver esa entrada). Debe tratar Economics como una dimensión **sin representación numérica propia** (ver `BOT-049.2`), no como una dimensión pendiente de completar.

  BOT-024 en sí (la definición formal de Signal Quality 0–100) sigue `TODO` — no se cierra hasta tener evidencia suficientemente validada (OOS) de al menos Momentum y Alignment. Su continuación pasa ahora por `BOT-050` → `BOT-051`, próximo trabajo activo: `BOT-050.1`.
