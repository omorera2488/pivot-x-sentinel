...
- **Subtareas/experimentos:**
  ...
  - `BOT-047` `IN PROGRESS / RESEARCH` — D1 Alignment (feature padre, ver "Convención para features experimentales" más abajo):
    ...
    - `BOT-047.3` `BLOCKED` — D1 Alignment Out-of-Sample Validation (espera revisión humana de los freezes `PROVISIONAL` de `BOT-047.2`, `BOT-047.2.1` y `BOT-047.2.2`).
...
- **Subtareas:**
  ...
  - `BOT-047.3` `BLOCKED` — OOS Validation (espera revisión humana explícita de los freezes `PROVISIONAL` de `BOT-047.2`, `BOT-047.2.1` y `BOT-047.2.2`, no se desbloquea automáticamente).
...
- **Siguiente paso planificado dentro de BOT-047 (bloqueado, ver `BOT-047.3`):** revisión humana explícita que decida qué hacer con el boundary — usar B0 tal cual, forzar B1, o adoptar una definición conservadora basada en el acuerdo entre ambos boundaries (candidato natural sugerido por esta tarea, no propuesto ni congelado aquí) — antes de desbloquear `BOT-047.3`.
