...
- **Subtareas/experimentos:**
  ...
  - `BOT-051` `IN PROGRESS / RESEARCH` — Signal Quality Integration (feature padre; se decidió, tras `BOT-051.1`, adoptar subestructura `.1/.2` dada la envergadura real del trabajo — ver esa entrada):
    ...
    - `BOT-051.2` `TODO — NEXT ACTIVE` — Signal Quality Definition Freeze (formalizar el esquema del vector de 4 slots; Momentum permanece `PENDING_DEFINITION`).

  BOT-024 en sí (la definición formal de Signal Quality 0–100) sigue `TODO` — no se cierra hasta tener evidencia suficientemente validada (OOS) de al menos Momentum y Alignment. Su continuación pasa ahora por `BOT-051.2`, próximo trabajo activo del proyecto tras el cierre de `BOT-051.1`.
...
- **Categoría:** Scoring / Investigación / Definición formal
- **Estado:** IN PROGRESS / RESEARCH — `BOT-051.1` `DONE` (decisión `VECTOR_FIRST`), `BOT-051.2` `TODO — NEXT ACTIVE`
- **Prioridad:** HIGH
- **Incorporado:** 2026-09-20
- **Versión objetivo:** N/A — investigación offline, sin release de producción
- **Descripción:** Integrar las dimensiones de `Signal Quality` (`Momentum` + `Alignment` + `Structure` + `Economics` + `Context`) preservando significado, trazabilidad e interpretabilidad de cada una — **no asume weighted scoring ni pesos iguales/arbitrarios**. Dirección futura de producto a mantener: `SIGNAL QUALITY: XX/100` con tabla/desglose visible de los factores y, si todos cumplen causalidad, disponible desde `limit_created_bar`. La transformación 0–100, los pesos, el manejo de confianza/`confidence` y las reglas de combinación deben definirse **posteriormente, con evidencia** — no en esta entrada ni de forma automática al completarse `BOT-049`/`BOT-050`. `BOT-051` **no introduce por sí mismo ningún gate en vivo**.
  ...
  - **Momentum (hallazgo de `BOT-051.1`, 2026-09-21):** a diferencia de Alignment/Structure/Context, Momentum (`BOT-024.2`) nunca tuvo un paso de Definition Freeze — solo Feature Discovery (`DONE`) y una OOS Validation `BLOCKED` antes de tener una hipótesis congelada que probar. `BOT-051.1` marca el slot de Momentum en el vector como **`PENDING_DEFINITION`**, no lo bloquea ni lo inventa. Ver `reports/BOT-051.1-signal-quality-integration-design.md` sección 3/§9/§12 para el detalle.
- **Dependencias:** `BOT-024.2`/`BOT-024.3` (Momentum, sin Definition Freeze — gap documentado en `BOT-051.1`), `BOT-047` (Alignment, `FREEZE_READY`), `BOT-048` (Structure, `ORIGIN_ONLY_FREEZE`), `BOT-049` (Economics, `DONE — NO_VALID_ECONOMICS_FREEZE`, sin representación que integrar), `BOT-050` (Context, `TEMPORAL_ONLY_FREEZE`).
...
- **Subtareas:**
  ...
  - `BOT-051.2` `TODO — NEXT ACTIVE` — Signal Quality Definition Freeze.
...
- **Incorporado:** 2026-09-21
- **Versión objetivo:** N/A — investigación offline, sin release de producción
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.1`. Debe formalizar el esquema tipado del vector `VECTOR_FIRST` decidido en `BOT-051.1` (`reports/BOT-051.1-signal-quality-integration-design.md` sección 15): estructura con campos `momentum` (`PENDING_DEFINITION`), `alignment` (enum 6 estados), `structure` (float continuo), `context` (enum 7 días), más `direction` como metadato obligatorio adjunto. **NO** debe poblar `momentum` con ningún candidato de `BOT-024.2` (requiere un Definition Freeze propio primero, fuera de alcance salvo decisión explícita). **NO** debe avanzar a Candidate B/C/D (composite state / ordinal / score 0-100) hasta que exista Momentum Definition Freeze y al menos una validación OOS genuina (`BOT-047.3`/`BOT-048.3`/`BOT-050.3`).
- **Dependencias:** BOT-051.1 (`DONE`, decisión `VECTOR_FIRST` y semántica de missing de dos niveles a respetar).
...
- **F. Signal Quality Integration (`BOT-051`) no exige cinco representaciones numéricas obligatorias — actualizado 2026-09-21 tras `BOT-051.1`:** tabla de estado actual — Momentum (`BOT-024.2` `DONE`, **sin Definition Freeze**, OOS `BLOCKED`; slot del vector marcado `PENDING_DEFINITION`), Alignment (`BOT-047.2.3` `FREEZE_READY`, OOS `READY/WAITING`), Structure (`BOT-048.2` `ORIGIN_ONLY_FREEZE`, OOS `PENDING/WAITING`), Economics (`BOT-049.2` `NO_VALID_ECONOMICS_FREEZE` — **excluida del vector, `EXCLUDED_ABSORBED`, no pendiente**), Context (`BOT-050.2` `TEMPORAL_ONLY_FREEZE` — **representación parcial**: solo `weekday`/Viernes, OOS `BLOCKED_WAITING_GENUINE_NEW_DATA`). `BOT-051.1` decidió `VECTOR_FIRST`: vector de 4 slots nominales (no 5), 3 poblables hoy, Momentum pendiente de su propio freeze. La integración futura debe usar únicamente los factores que sobrevivan sus propios contratos/validaciones — sin pesos, score 0–100, `confidence` ni gate definidos todavía (ver `BOT-051`, `BOT-051.1`, `BOT-051.2`).
