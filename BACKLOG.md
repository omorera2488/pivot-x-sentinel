...
- **Subtareas/experimentos:**
  ...
  - `BOT-051` `IN PROGRESS / RESEARCH` — Signal Quality Integration (feature padre; adopta subestructura `.1/.2/.3/.4` — ver esa entrada):
    ...
    - `BOT-051.4` `TODO — NEXT ACTIVE` — Signal Quality Live Persistence (extensión de `execution/src/score_store.py`, downstream, no offline puro).

  BOT-024 en sí (la definición formal de Signal Quality 0–100) sigue `TODO` — no se cierra hasta tener evidencia suficientemente validada (OOS) de al menos Momentum y Alignment. Su continuación pasa ahora por `BOT-051.4`, próximo trabajo activo del proyecto tras el cierre de `BOT-051.3`.
...
- **Categoría:** Scoring / Investigación / Definición formal
- **Estado:** IN PROGRESS / RESEARCH — `BOT-051.1` `DONE` (decisión `VECTOR_FIRST`), `BOT-051.2` `DONE` (decisión `SIGNAL_QUALITY_VECTOR_V1_FREEZE`), `BOT-051.3` `DONE` (decisión `SHADOW_OBSERVABILITY_READY`), `BOT-051.4` `TODO — NEXT ACTIVE`
- **Prioridad:** HIGH
- **Incorporado:** 2026-09-20
- **Versión objetivo:** N/A — investigación offline, sin release de producción
...
- **Subtareas:**
  ...
  - `BOT-051.4` `TODO — NEXT ACTIVE` — Signal Quality Live Persistence (extensión de `execution/src/score_store.py`, evaluar introducir ATR-Wilder en producción antes de avanzar).
...
- **Incorporado:** 2026-09-21
- **Versión objetivo:** a definir — es la primera subtarea de esta línea que tocaría `execution/`/`api/`/potencialmente `panel/`, no offline puro como `.1`/`.2`/`.3`.
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.3`. `BOT-051.3` dejó `SHADOW_OBSERVABILITY_READY` pero explícitamente **no** cableó el productor del vector (`scripts/signal_quality_vector.py`) a producción, porque Momentum/Structure dependen de ATR-Wilder y esa función **no existe en ningún archivo de `strategy/` hoy** (confirmado por grep exhaustivo). Esta tarea debe: (1) evaluar si conviene introducir ATR-Wilder como dependencia de producción (o una alternativa que reutilice mejor primitivas ya existentes de `strategy/engine.py`), (2) solo si esa evaluación es favorable, extender `execution/src/score_store.py::record()` (el único punto de persistencia por-LIMIT ya existente, escrito exactamente en el momento de creación vía `execution/src/bot.py:682`) para incluir el vector junto al `score` ya persistido, (3) exponerlo en `/scores` (`api/app.py`), y (4) opcionalmente en `panel/` como vector crudo (sin score, sin colores, sin tiers — mismas restricciones que `BOT-051.1`/`.2`/`.3`; `panel/app.js` ya tolera campos faltantes, `panel/app.js:143-145`). Requiere su propia prueba de behavior invariance (a diferencia de `.1`/`.2`/`.3`, que la obtuvieron gratis al no tocar producción). **No** inicia investigación de score 0–100 ni gate — sigue fuera de alcance hasta que exista validación OOS suficiente.
...
- **F. Signal Quality Integration (`BOT-051`) no exige cinco representaciones numéricas obligatorias — actualizado 2026-09-21 tras `BOT-051.3`:** tabla de estado actual — Momentum (`BOT-024.2` `DONE`, `BOT-024.4` `FREEZE_READY` — canónica `roc_atr_3`, RAW; OOS `BOT-024.3` `BLOCKED`), Alignment (`BOT-047.2.3` `FREEZE_READY`, OOS `READY/WAITING`), Structure (`BOT-048.2` `ORIGIN_ONLY_FREEZE`, OOS `PENDING/WAITING`), Economics (`BOT-049.2` `NO_VALID_ECONOMICS_FREEZE` — **excluida del vector, `EXCLUDED_ABSORBED`, no pendiente**), Context (`BOT-050.2` `TEMPORAL_ONLY_FREEZE` — **representación parcial**: solo `weekday`/Viernes, OOS `BLOCKED_WAITING_GENUINE_NEW_DATA`). **`BOT-051.2` congeló el esquema `SignalQualityVectorV1`** (decisión `SIGNAL_QUALITY_VECTOR_V1_FREEZE`) y **`BOT-051.3` lo hizo observable en shadow sobre el 100% de Universo A** (decisión `SHADOW_OBSERVABILITY_READY`: 3.207/3.207 LIMITs con vector producido, 3.180/3.207 con los 4 factores `AVAILABLE`, productor tipado e inmutable implementado en `scripts/signal_quality_vector.py`, cero cambios de producción) — vector de 4 slots nominales (no 5), los 4 con Definition Freeze propio, ninguno OOS-validado todavía. La exposición en vivo (`score_store`/`/scores`/`panel`) queda deliberadamente diferida a `BOT-051.4`. La integración futura debe usar únicamente los factores que sobrevivan sus propios contratos/validaciones — sin pesos, score 0–100, `confidence` ni gate definidos todavía (ver `BOT-051`, `BOT-051.1`, `BOT-051.2`, `BOT-051.3`, `BOT-051.4`).
