...
**IDs:** secuenciales, nunca se reutilizan aunque el ítem se cancele. Última ID **principal** usada: **BOT-052**. Siguiente disponible: **BOT-053**. Los IDs decimales (`BOT-XXX.1`, `.2`, `.3`, ...) no consumen IDs principales nuevos — ver "Convención para features experimentales" más abajo.
...
*(Última actualización: 2026-09-24, ver BOT-052/BOT-052.1/BOT-052.2/BOT-024.2/BOT-024.3/BOT-024.4/BOT-047/BOT-047.1/BOT-047.2/BOT-047.3/BOT-048/BOT-048.1/BOT-048.2/BOT-049/BOT-049.1/BOT-049.2/BOT-049.3/BOT-050/BOT-051/BOT-051.1/BOT-051.2/BOT-051.3/BOT-051.4/BOT-051.5/BOT-051.6/BOT-051.6.1/BOT-051.6.2/BOT-051.6.3/BOT-051.6.4/BOT-051.7.)* Esta sección representa el **orden operativo recomendado** — puede diferir de la prioridad intrínseca (`CRITICAL`/`HIGH`/`MEDIUM`/`LOW`) de cada ítem, que no se modifica solo para coincidir con este orden.

1. **BOT-024 — Signal Quality** (línea de investigación activa, `HIGH`): ~~**BOT-024.1** — Evaluación predictiva de Divergencia RSI~~ `DONE`. ~~**BOT-024.2** — Momentum Feature Discovery XAU~~ `DONE` — ver `reports/BOT-024.2-MOMENTUM-FEATURE-DISCOVERY.md`. **BOT-024.3** — Momentum Out-of-Sample Validation `BLOCKED — waiting for genuine OOS` (no existe histórico posterior al 2026-09-15 en el repositorio, ver `reports/BOT-024.3-MOMENTUM-OOS.md`). **BOT-047 — D1 Alignment** (feature padre, `IN PROGRESS / RESEARCH`): ~~**BOT-047.1 — D1 Alignment Feature Discovery**~~ `DONE` — ver `reports/BOT-047.1-D1-ALIGNMENT-FEATURE-DISCOVERY.md`. ~~**BOT-047.2 — D1 Alignment Definition Freeze**~~ `DONE` — resultado **`PROVISIONAL`** (no `FREEZE_READY`), ver `reports/BOT-047.2-ALIGNMENT-DEFINITION-FREEZE.md`. `closed_ema200_slope` se trata provisionalmente como **D1 Regime / Trend Strength**, no como definición final de Alignment. ~~**BOT-047.2.1 — Structural Alignment Definition & Legacy Decomposition**~~ `DONE` — resultado **`PROVISIONAL`**, ver `reports/BOT-047.2.1-STRUCTURAL-ALIGNMENT-DEFINITION.md`. ~~**BOT-047.2.2 — D1 Structural Boundary Validation**~~ `DONE` — resultado **`PROVISIONAL`**, ver `reports/BOT-047.2.2-D1-STRUCTURAL-BOUNDARY-VALIDATION.md`. ~~**BOT-047.2.3 — Structural Alignment Consensus Definition Freeze**~~ `DONE` — resultado **`FREEZE_READY`** (definición congelada, no validada), ver `reports/BOT-047.2.3-STRUCTURAL-ALIGNMENT-CONSENSUS-FREEZE.md`. **BOT-047.3 — D1 Alignment OOS Validation** `READY / WAITING FOR OOS` — la decisión metodológica ya está tomada (Structural Alignment Consensus, congelado en `BOT-047.2.3`), pero sigue sin ejecutarse porque no existe histórico posterior al 2026-09-15 en el repositorio (mismo motivo que `BOT-024.3`). **BOT-048 — Structure** (feature padre, `IN PROGRESS / RESEARCH`): ~~**BOT-048.1 — Structure Feature Discovery**~~ `DONE` — ver `reports/BOT-048.1-STRUCTURE-FEATURE-DISCOVERY.md`. ~~**BOT-048.2 — Structure Definition Freeze**~~ `DONE` — resultado **`ORIGIN_ONLY_FREEZE`** (contrato RAW continuo sobre `origin_dist_atr`; Swing y Forward-Space evaluados y rechazados, Structural Consensus rechazado por falta de frontera defendible, `origin_retracement_frac` clasificado `CROSS_FACTOR_ONLY`), ver `reports/BOT-048.2-STRUCTURE-DEFINITION-FREEZE.md`. **BOT-048.3 — Structure OOS Validation** `PENDING / WAITING OOS` — contrato pre-registrado en `BOT-048.2`, sigue sin ejecutarse porque no existe histórico posterior al 2026-09-15 en el repositorio (mismo motivo que `BOT-024.3`/`BOT-047.3`). **Roadmap acordado para completar Signal Quality** (formalizado 2026-09-20): **BOT-049 — Economics** → **BOT-050 — Context** → **BOT-051 — Signal Quality Integration** → **BOT-025** (gate/decisión de ejecución, downstream). ~~**BOT-049.1 — Economics Feature Discovery**~~ `DONE` (2026-09-21) — ver `reports/BOT-049.1-ECONOMICS-FEATURE-DISCOVERY.md`. ~~**BOT-049.2 — Economics Definition Freeze**~~ `DONE` (2026-09-21) — resultado **`NO_VALID_ECONOMICS_FREEZE`**: bajo Config A y el diseño actual de la estrategia, ninguna candidata de Economics sobrevive con ownership propio — `risk_atr` es casi idéntico algebraicamente a `origin_dist_atr` de Structure (ρ=0.976–0.982, verificado exhaustivamente sobre las 3.207 filas, `REJECT_REDUNDANT`), la fricción de spread (`spread_over_risk`/`breakeven_pct`/`rr_effective_net`) es la MISMA cantidad que el término de costo de `cvp_score()` (verificado bit-exacto sobre las 3.207 filas, ownership asignado a CVP), `distance_to_limit_atr` se reclasificó como execution/fill-quality (no Economics), `origin_retracement_frac` reconfirmado `CROSS_FACTOR_ONLY` (sin error nuevo encontrado), y `rr_nominal/rr_geometric` siguen `REJECTED` (degenerados). Ver `reports/BOT-049.2-ECONOMICS-DEFINITION-FREEZE.md`. **`BOT-049` (feature padre) pasa a `DONE — NO_VALID_ECONOMICS_FREEZE`** — no queda ninguna dimensión Economics independiente para validar OOS bajo la evidencia actual; `BOT-049.3` pasa a `TODO — NO CONTRACT TO VALIDATE` (no hay definición congelada que validar, distinto de estar bloqueada por falta de datos OOS). ~~**BOT-050.1** — Context Feature Discovery~~ `DONE` (2026-09-21) — ver `reports/BOT-050.1-CONTEXT-FEATURE-DISCOVERY.md`. ~~**BOT-050.2** — Context Definition Freeze~~ `DONE` (2026-09-21) — resultado **`TEMPORAL_ONLY_FREEZE`**: solo `weekday`/efecto Viernes sobrevive ownership independiente frente a Momentum/Alignment/Structure (verificado con un modelo diagnóstico conjunto, no solo condicionamiento uno-a-la-vez); `atr_ratio_short_long` (volatilidad relativa ATR14/160, ligada al mecanismo HTF de Config A) queda `CROSS_FACTOR_ONLY` pese a evidencia causal/semántica sólida, por perder esa contribución marginal en el modelo conjunto; `session_utc`/Asia queda `CROSS_FACTOR_ONLY` (limitación DST). Ver `reports/BOT-050.2-CONTEXT-DEFINITION-FREEZE.md`. **`BOT-050` (feature padre) pasa a `DONE — TEMPORAL_ONLY_FREEZE`**; `BOT-050.3` pasa a `TODO — BLOCKED_WAITING_GENUINE_NEW_DATA` (contrato congelado, esperando histórico genuinamente posterior a 2026-09-15). ~~**BOT-051.1 — Signal Quality Integration Design / Contract Discovery**~~ `DONE` (2026-09-21) — decisión **`VECTOR_FIRST`**: vector de 4 slots (Momentum/Alignment/Structure/Context, con `direction` como metadato adjunto), de los cuales 3 tenían contrato congelado y reconstruible ese día (Alignment `FREEZE_READY`, Structure `ORIGIN_ONLY_FREEZE`, Context `TEMPORAL_ONLY_FREEZE`) y 1 quedó **`PENDING_DEFINITION`** (Momentum — único factor sin Definition Freeze propio, gap detectado explícitamente en esta tarea). Economics confirmado excluido (`EXCLUDED_ABSORBED`, no pendiente). Ver `reports/BOT-051.1-signal-quality-integration-design.md`. ~~**BOT-024.4 — Momentum Definition Freeze**~~ `DONE` (2026-09-21, sesión separada del usuario) — cerró ese gap: **`FREEZE_READY`**, canónica `roc_atr_3` (RAW continua), ver `reports/BOT-024.4-MOMENTUM-DEFINITION-FREEZE.md`. ~~**BOT-051.2 — Signal Quality Definition Freeze**~~ `DONE` (2026-09-21) — decisión **`SIGNAL_QUALITY_VECTOR_V1_FREEZE`**: esquema tipado `SignalQualityVectorV1` congelado (los 4 slots ya poblables, ninguno OOS-validado), reconstrucción causal 2.474/2.474 exitosa (universo acotado por el artefacto de Alignment, N=2.474, no por contrato), 2.456/2.474 eventos con los 4 factores `AVAILABLE`, 17 invariantes congelados, sin redundancia fuerte entre los 4 contratos. Ver `reports/BOT-051.2-signal-quality-definition-freeze.md` y `reports/BOT-051.2-signal-quality-contract.json`. ~~**BOT-051.3 — Signal Quality Vector Shadow Reconstruction / Observability**~~ `DONE` (2026-09-21) — decisión **`SHADOW_OBSERVABILITY_READY`**: cerró la brecha de Universo A (Alignment reconstruido para los 3.207 LIMITs, no solo los 2.474 de Universo B, reusando sin modificar las funciones ya congeladas de `BOT-047.2.1`; parity 100% contra el artefacto previo), cerró el re-slice causal standalone de Momentum pendiente de `BOT-024.4` (exhaustivo, 0 discrepancias sobre 3.207 eventos), implementó el productor tipado e inmutable `SignalQualityVectorV1` (`scripts/signal_quality_vector.py`), y produjo **3.207/3.207 vectores** (3.180 con los 4 factores `AVAILABLE`) sin tocar `strategy/`/`execution/`/`api/`/`panel/`. Decidió explícitamente diferir la exposición en vivo (`score_store`/`/scores`/`panel`) a `BOT-051.4`, porque ATR-Wilder no existe en producción hoy. Ver `reports/BOT-051.3-signal-quality-vector-shadow-observability.md`. ~~**BOT-051.4 — Signal Quality Live Persistence / Production Observability**~~ `DONE` (2026-09-21) — decisión **`SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`**: Signal Quality ya se calcula y persiste dentro del bot productivo para cada LIMIT real, expuesto por `/scores` y visible en el panel — parity exhaustiva sin discrepancias contra el histórico completo (N=3.207: `roc_atr_3`/`origin_dist_atr` vs `BOT-024.2`/`BOT-048.1`, `alignment`/`weekday` vs `BOT-051.3`), behavior invariance probada explícitamente (A/B con MT5 falso — request a `order_send()` byte-idéntico con o sin Signal Quality; `strategy/engine.py`/`live_signal.py` con diff cero). Ver `reports/BOT-051.4-signal-quality-live-persistence.md`. ~~**BOT-051.5 — Signal Quality Live Accumulation / OOS Bridge**~~ `DONE` (2026-09-22) — decisión **`SIGNAL_QUALITY_OOS_BRIDGE_READY`**, evidencia **`ACCUMULATING`**: pre-flight encontró y corrigió 5 líneas de contaminación de test mezcladas con datos reales en `execution/data/scores/` (causa raíz corregida, backup conservado), y confirmó **4 LIMITs reales genuinas** desde el deploy de `BOT-051.4` (el bot del usuario ya corrió contra la cuenta MT5 real) — reconciliadas read-only contra MT5 (1 `CANCELED`, 3 cerradas: 2 `SL`, 1 `TP`), identidad `order_ticket==position_id` verificada con datos reales, `OutcomeObservation` separado (`execution/src/outcome_store.py`, nunca mezclado con el snapshot inmutable de `t0`), telemetría de disponibilidad por factor agregada (`warmup`/`alignment_history`/`structure_replay_mismatch`/etc., sin tocar el contrato congelado), dataset OOS determinista y readiness por factor con criterios no arbitrarios (`NO_DATA`/`ACCUMULATING`/`READY_FOR_OOS_ANALYSIS` — los 4 factores quedan `ACCUMULATING` con la evidencia actual), API (`GET /signal-quality/oos-status`) y tarjeta de panel verificadas visualmente contra la cuenta real (bot permaneció detenido, cero riesgo). Ver `reports/BOT-051.5-signal-quality-live-accumulation-oos-bridge.md`. **`BOT-051` (feature padre) permanece `IN PROGRESS / RESEARCH`**. **Decisión de planificación (2026-09-22): no se detiene el desarrollo esperando volumen OOS** — la evidencia real sigue acumulándose en paralelo mientras el bot opera. `BOT-051.6` se reutilizó como **`Signal Quality Production UI / Observability UX`**, ~~`TODO — NEXT ACTIVE`~~ **`DONE` (2026-09-22) — decisión `SIGNAL_QUALITY_PRODUCTION_UI_READY`**: cerró el gap real de que Signal Quality ya se calculaba y persistía desde `BOT-051.4` pero solo era visible en el panel DESPUÉS de que el trade cerraba — ahora la tarjeta "Posiciones y pendientes" lista cada LIMIT pendiente y cada posición abierta con el mismo ícono/popover ya existente (reutilizado, no una superficie nueva), verificado con una posición real (#351122611) contra la cuenta MT5 real conectada. También cerró un segundo gap: `score_store.load_all()` (fuente de `/scores`) nunca exponía `signal_quality_diagnostics` (la razón de `UNAVAILABLE`, agregada en `BOT-051.5`) — el panel ahora la muestra (`Reason: <razón>`). Separación t0/outcome reforzada con título explícito y probada a nivel de datos (snapshot byte-idéntico tras reconciliar 3 outcomes). Bug de overflow del popover encontrado y corregido durante la validación visual (centrado + clamp de ancho, reverificado en 3 anchos de viewport). 17 tests nuevos + regresión completa sin cambios, `strategy/engine.py`/`strategy/live_signal.py` diff cero. Ver `reports/BOT-051.6-signal-quality-production-ui-observability-ux.md`. Un hotfix inmediato, **`BOT-051.6.1 — Signal Quality Production UI Cleanup`**, `DONE` (2026-09-22) — decisión **`SIGNAL_QUALITY_PRODUCTION_UI_CLEANUP_READY`**: la validación visual posterior mostró que el popover seguía mostrando la calificación legacy (Divergencia/Tendencia/CVP/Nodo/Total) por encima de Signal Quality v1 — corregido, el popover queda exclusivamente con Signal Quality v1 (sin borrar el dato legacy, que se sigue calculando/persistiendo igual, solo sale de la superficie productiva principal); la tarjeta OOS pasa a ocupar el ancho completo; de paso se encontró y corrigió un bug real preexistente de grid CSS (afectaba a TODAS las tarjetas full-width, no solo Signal Quality) que dejaba espacio vacío en un breakpoint intermedio. Ver `reports/BOT-051.6.1-signal-quality-production-ui-cleanup.md`. Una auditoría posterior read-only, **`BOT-051.6.2 — Signal Quality UI / Lifecycle Observability Audit`**, `DONE` (2026-09-23) — decisión **`PERSISTENCE_FAILURE_IDENTIFIED`**: investigó por qué una LIMIT real (`#351931195`) no mostraba Signal Quality ni abierta ni cerrada, descartando explícitamente bugs de UI/lookup/identidad MT5 y encontrando la causa real: un `numpy.bool_` sin castear en `strategy/signal_quality.py::_replay_armado_origin()` rompía `json.dumps()` en `score_store.record()` para toda señal real donde Structure replay confirmara la señal (el caso normal) — confirmado con el log real del bot en vivo, activo en producción en ese momento (afectaba también a una posición que seguía abierta, `#352047367`). Ver `reports/BOT-051.6.2-signal-quality-ui-lifecycle-audit.md`. El hotfix derivado, **`BOT-051.6.3 — Signal Quality Persistence Serialization Hotfix`**, `DONE` (2026-09-23) — decisión **`SIGNAL_QUALITY_PERSISTENCE_HOTFIX_READY`**: corrigió el tipo en el productor (`bool(...)` explícito, sin reabrir ningún contrato), agregó tests con arrays NumPy reales que cubren específicamente el escenario `replay_matches=True`, agregó una defensa en profundidad explícita y acotada en `score_store.py`, y aisló `score_store.record()` en `execution/src/bot.py` para que un fallo de persistencia nunca vuelva a tumbar el ciclo principal ni a arriesgar el reprocesamiento de una barra — los tickets perdidos NO se backfillearon (sin fabricar evidencia OOS retrospectiva). Ver `reports/BOT-051.6.3-signal-quality-persistence-serialization-hotfix.md`. Un hotfix de presentación final, **`BOT-051.6.4 — Restore Signal Quality in Positions & Pending UI`**, `DONE` (2026-09-23) — decisión **`SIGNAL_QUALITY_LIFECYCLE_UI_RESTORED`**: el P&L flotante por posición se mostraba duplicado (ya vivía en la tarjeta `FLOTANTE`) en el extremo derecho de cada fila de "Posiciones y pendientes" — corregido, ese extremo vuelve a ser exclusivamente Signal Quality (mismo `scoreBadge()` reutilizado, sin segunda implementación), con un placeholder neutral ("SQ no disponible") para tickets sin snapshot genuino. Auditoría previa (sin encontrar ningún commit que hubiera "reemplazado" el badge — nunca existió tal reemplazo, ambos coexistieron desde `BOT-051.6`) documentada para evitar la misma confusión a futuro. Ver `reports/BOT-051.6.4-restore-signal-quality-positions-pending-ui.md`. El antiguo `BOT-051.6 — Signal Quality OOS Readiness Review` se renumera a **`BOT-051.7`**, `TODO — WAITING FOR OOS READINESS` (`LOW`) — permanece en espera hasta que `execution/src/signal_quality_oos_dataset.py::readiness()` reporte `READY_FOR_OOS_ANALYSIS` para algún factor; no se relajan los criterios ya codificados para acelerarlo. **Actualización orgánica (observada durante `BOT-051.6.4`, sin intervención de esa tarea):** `Momentum` y `Context` ya cruzaron a `READY_FOR_OOS_ANALYSIS` con evidencia real (7 LIMITs genuinas, 6 cerradas) — `BOT-051.7` sigue sin abrirse automáticamente, queda como buen candidato para retomarse pronto. BOT-024 en sí (definición formal de Signal Quality 0–100) permanece `TODO`. **La acumulación OOS ocurre en paralelo al desarrollo normal del bot — el bajo número de operaciones reales por día y los períodos en que el bot permanece apagado pueden hacer que alcanzar cobertura suficiente tome tiempo; esto no bloquea trabajo de ingeniería, UI, observabilidad, documentación o preparación que no dependa estadísticamente del OOS.**
   **BOT-052 — Confluence Reader Evaluation** (feature padre, `IN PROGRESS / RESEARCH`): ~~**BOT-052.1 — Confluence Reader Historical Discovery**~~ `DONE` (2026-09-24) — `D` `WEAK_OR_UNSTABLE`, `HCH` `INSUFFICIENT_SAMPLE`, `D+HCH` `WEAK_OR_UNSTABLE`, EMA `WEAK_OR_UNSTABLE` (E3 `REDUNDANT`), ver `reports/BOT-052.1-confluence-reader-historical-discovery.md`. ~~**BOT-052.2 — HCH Definition Freeze + Live Shadow Observability**~~ `DONE` (2026-09-24) — decisión **`HCH_SHADOW_FREEZE`**: definición causal de HCH de `BOT-052.1` congelada en una única implementación canónica (`strategy/hch.py::HCHEngine`), capturada en vivo como telemetría shadow pura en cada LIMIT (persistida junto a `signal_quality`, joineable por diseño), paridad histórica EXACTA (Universo A 3.207=234 HCH+2.972 NO_HCH+1 UNAVAILABLE, Universo B 2.474=161 HCH+2.313 NO_HCH), `TRADING_BEHAVIOR_PARITY = EXACT` (`git diff` vacío en `strategy/engine.py`/`live_signal.py`), `NO_GATE`/`NO_SCORE_CHANGE`/`NO_EXECUTION_CHANGE`. Ver `reports/BOT-052.2-hch-definition-freeze-live-shadow.md`. **BOT-052.3 — HCH OOS Validation + Signal Quality Interaction Analysis** `TODO / BLOCKED_ON_OOS` (0 observaciones en vivo al cierre de `BOT-052.2`, mismo tipo de barrera que `BOT-024.3`/`BOT-047.3`/`BOT-048.3`).
...
- **Subtareas/experimentos:**
  ...
  - `BOT-051` `IN PROGRESS / RESEARCH` — Signal Quality Integration (feature padre; adopta subestructura `.1/.2/.3/.4/.5/.6/.6.1/.7` — ver esa entrada):
    ...
    - `BOT-051.3` `DONE` — Signal Quality Vector Shadow Reconstruction / Observability (decisión: `SHADOW_OBSERVABILITY_READY`, ver `reports/BOT-051.3-signal-quality-vector-shadow-observability.md`).
    - `BOT-051.4` `DONE` — Signal Quality Live Persistence / Production Observability (decisión: `SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`, ver `reports/BOT-051.4-signal-quality-live-persistence.md`).
    - `BOT-051.5` `DONE` — Signal Quality Live Accumulation / OOS Bridge (decisión: `SIGNAL_QUALITY_OOS_BRIDGE_READY`, evidencia `ACCUMULATING`, ver `reports/BOT-051.5-signal-quality-live-accumulation-oos-bridge.md`).
    - `BOT-051.6` `DONE` — Signal Quality Production UI / Observability UX (decisión: `SIGNAL_QUALITY_PRODUCTION_UI_READY`, ver `reports/BOT-051.6-signal-quality-production-ui-observability-ux.md`).
    - `BOT-051.6.1` `DONE` — Signal Quality Production UI Cleanup (decisión: `SIGNAL_QUALITY_PRODUCTION_UI_CLEANUP_READY`, hotfix de presentación — elimina la calificación legacy del popover productivo, ver `reports/BOT-051.6.1-signal-quality-production-ui-cleanup.md`).
    - `BOT-051.6.2` `DONE` — Signal Quality UI / Lifecycle Observability Audit (decisión: `PERSISTENCE_FAILURE_IDENTIFIED`, ver `reports/BOT-051.6.2-signal-quality-ui-lifecycle-audit.md`).
    - `BOT-051.6.3` `DONE` — Signal Quality Persistence Serialization Hotfix (decisión: `SIGNAL_QUALITY_PERSISTENCE_HOTFIX_READY`, ver `reports/BOT-051.6.3-signal-quality-persistence-serialization-hotfix.md`).
    - `BOT-051.6.4` `DONE` — Restore Signal Quality in Positions & Pending UI (decisión: `SIGNAL_QUALITY_LIFECYCLE_UI_RESTORED`, ver `reports/BOT-051.6.4-restore-signal-quality-positions-pending-ui.md`).
    - `BOT-051.7` `TODO — WAITING FOR OOS READINESS` — Signal Quality OOS Readiness Review (antes numerada `BOT-051.6`).

  BOT-024 en sí (la definición formal de Signal Quality 0–100) sigue `TODO` — no se cierra hasta tener evidencia suficientemente validada (OOS) de al menos Momentum y Alignment. Su continuación pasa ahora por `BOT-051.7` (OOS Readiness Review, sigue `WAITING FOR OOS READINESS` — evidencia real ACCUMULATING, no se fuerza), tras el cierre de `BOT-051.6` — no se espera al volumen OOS para seguir construyendo (BOT-051.6 ya hizo el Signal Quality calculado/persistido desde BOT-051.4 visible/legible en el panel desde que nace la LIMIT, no solo al cerrar el trade).
...
- **Categoría:** Scoring / Investigación / Definición formal
- **Estado:** IN PROGRESS / RESEARCH — `BOT-051.1` `DONE` (decisión `VECTOR_FIRST`), `BOT-051.2` `DONE` (decisión `SIGNAL_QUALITY_VECTOR_V1_FREEZE`), `BOT-051.3` `DONE` (decisión `SHADOW_OBSERVABILITY_READY`), `BOT-051.4` `DONE` (decisión `SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`), `BOT-051.5` `DONE` (decisión `SIGNAL_QUALITY_OOS_BRIDGE_READY`, evidencia `ACCUMULATING`), `BOT-051.6` `DONE` (decisión `SIGNAL_QUALITY_PRODUCTION_UI_READY`), `BOT-051.6.1` `DONE` (decisión `SIGNAL_QUALITY_PRODUCTION_UI_CLEANUP_READY`, hotfix de presentación), `BOT-051.6.2` `DONE` (decisión `PERSISTENCE_FAILURE_IDENTIFIED`, auditoría read-only), `BOT-051.6.3` `DONE` (decisión `SIGNAL_QUALITY_PERSISTENCE_HOTFIX_READY`, hotfix de persistencia — root cause `numpy.bool_` corregido), `BOT-051.6.4` `DONE` (decisión `SIGNAL_QUALITY_LIFECYCLE_UI_RESTORED`, P&L flotante deduplicado de "Posiciones y pendientes"), `BOT-051.7` `TODO — WAITING FOR OOS READINESS` (OOS Readiness Review, antes `BOT-051.6`) — NEXT: `Momentum`/`Context` ya cruzaron a `READY_FOR_OOS_ANALYSIS` orgánicamente (observado durante `BOT-051.6.4`), buen candidato para retomar pronto, sin abrirse automáticamente
- **Prioridad:** HIGH
- **Incorporado:** 2026-09-20
- **Versión objetivo:** N/A — investigación offline, sin release de producción (`BOT-051.4` fue la primera subtarea que SÍ tocó `strategy/`/`execution/`/`api/`/`panel/`, ver esa entrada)
- **Descripción:** Integrar las dimensiones de `Signal Quality` (`Momentum` + `Alignment` + `Structure` + `Economics` + `Context`) preservando significado, trazabilidad e interpretabilidad de cada una — **no asume weighted scoring ni pesos iguales/arbitrarios**. Dirección futura de producto a mantener: `SIGNAL QUALITY: XX/100` con tabla/desglose visible de los factores y, si todos cumplen causalidad, disponible desde `limit_created_bar`. La transformación 0–100, los pesos, el manejo de confianza/`confidence` y las reglas de combinación deben definirse **posteriormente, con evidencia** — no en esta entrada ni de forma automática al completarse `BOT-049`/`BOT-050`. `BOT-051` **no introduce por sí mismo ningún gate en vivo**. Desde `BOT-051.4` (2026-09-21), Signal Quality **existe y se ve dentro del bot productivo** (observacional, sin participar en decisiones de trading) — ver esa entrada.
  - **Estructura interna (actualizado 2026-09-21 tras `BOT-051.1`, extendida 2026-09-22 con `.6`/`.7`):** SÍ adopta subestructura `.1/.2` (se revierte la expectativa original de "ID principal sin subestructura" — el trabajo real de *contract discovery* resultó suficientemente extenso como para justificarlo, igual que `BOT-047`/`BOT-048`/`BOT-049`/`BOT-050`). No se usa `.3` (OOS) porque `BOT-051` no es una feature con su propio discovery estadístico de datos nuevos — es integración de contratos ya congelados por otras líneas; la validación OOS de cada factor individual sigue viviendo en `BOT-047.3`/`BOT-048.3`/`BOT-050.3`.
  - **Estrategia de acumulación OOS (2026-09-22):** la acumulación OOS ocurre en paralelo al desarrollo normal del bot. El bajo número de operaciones reales por día y los períodos en que el bot permanece apagado pueden hacer que alcanzar cobertura suficiente tome tiempo; esto **no bloquea** trabajo de ingeniería, UI, observabilidad, documentación o preparación que no dependa estadísticamente del OOS — por eso `BOT-051.6` (Production UI, cerrado 2026-09-22) se completó mientras `BOT-051.7` (antes numerada `BOT-051.6`, OOS Readiness Review) sigue en espera. No se estiman fechas artificiales ni se relajan los criterios de readiness ya codificados en `BOT-051.5` para acelerarla.
...
- **Subtareas:**
  ...
  - `BOT-051.3` `DONE` — Signal Quality Vector Shadow Reconstruction / Observability (decisión `SHADOW_OBSERVABILITY_READY`).
  - `BOT-051.4` `DONE` — Signal Quality Live Persistence / Production Observability (decisión `SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`).
  - `BOT-051.5` `DONE` — Signal Quality Live Accumulation / OOS Bridge (decisión `SIGNAL_QUALITY_OOS_BRIDGE_READY`, evidencia `ACCUMULATING`).
  - `BOT-051.6` `DONE` — Signal Quality Production UI / Observability UX (decisión `SIGNAL_QUALITY_PRODUCTION_UI_READY`).
  - `BOT-051.6.1` `DONE` — Signal Quality Production UI Cleanup (decisión `SIGNAL_QUALITY_PRODUCTION_UI_CLEANUP_READY`, hotfix de presentación).
  - `BOT-051.6.2` `DONE` — Signal Quality UI / Lifecycle Observability Audit (decisión `PERSISTENCE_FAILURE_IDENTIFIED`, auditoría read-only).
  - `BOT-051.6.3` `DONE` — Signal Quality Persistence Serialization Hotfix (decisión `SIGNAL_QUALITY_PERSISTENCE_HOTFIX_READY`).
  - `BOT-051.6.4` `DONE` — Restore Signal Quality in Positions & Pending UI (decisión `SIGNAL_QUALITY_LIFECYCLE_UI_RESTORED`).
  - `BOT-051.7` `TODO — WAITING FOR OOS READINESS` — Signal Quality OOS Readiness Review (renumerada desde `BOT-051.6`, ver esa entrada para la razón).
...
- **Incorporado:** 2026-09-21
- **Versión objetivo:** N/A — investigación offline, sin release de producción
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.2`. Objetivo: producir el vector `SignalQualityVectorV1` (congelado en `BOT-051.2`) en modo shadow sobre el universo histórico más amplio posible, verificar estabilidad/reconstructibilidad, cerrar la brecha de re-slice causal de Momentum pendiente desde `BOT-024.4`, e implementar un productor tipado del vector. **Sin score, sin peso, sin threshold, sin gate.**
- **Resultado (2026-09-21):** **`SHADOW_OBSERVABILITY_READY`.** **Cierre completo de la brecha de Universo A dejada por `BOT-051.2`** (`scripts/reconstruct_signal_quality_vector_full_universe_xau.py`): las funciones ya congeladas de Alignment (`build_naive_blocks`/`naive_trend_at`/`build_true_blocks`/`true_trend_at`/`alignment_label`, `BOT-047.2.1`, reusadas sin modificar) operan puramente sobre `bar_idx`, sin depender de si el evento llegó a fill — se evaluaron los 3.207 eventos de Universo A completo (no solo los 2.474 de Universo B). **Distribución de Consensus sobre Universo A (N=3.207):** `NEUTRAL`=1.593, `AGAINST_SINGLE`=594, `ALIGNED_SINGLE`=565, `AGAINST_CONFIRMED`=227, `ALIGNED_CONFIRMED`=201, `UNAVAILABLE`=27 (0,84%, mismo tipo de causa —insuficiente historia D1 en eventos tempranos— que el 0,73% ya documentado sobre Universo B). **Parity 100%** contra el artefacto previo de `BOT-047.2.2` (2.474/2.474 `B0_alignment`/`B1_alignment` exact-match, 0 mismatches). **Cierre del re-slice causal standalone de Momentum** (limitación #3 pendiente de `BOT-024.4`): recómputo exhaustivo (no muestreado) de `roc_atr_3` con historia truncada en `limit_created_bar` sobre los 3.207 eventos — 3.206 verificados, 1 warm-up legítimo, **0 discrepancias**, error absoluto máximo 4,44e-16 (ruido de punto flotante). **Vector conjunto sobre Universo A completo:** joins Momentum/Structure/Context 3.207/3.207, **3.207/3.207 vectores producidos (0 fallos de reconstrucción)**, 3.180/3.207 con los 4 factores `AVAILABLE`, 27/3.207 con `UNAVAILABLE` (100% Alignment) — incluye los 733 LIMITs que nunca llegaron a fill, con la misma validez que los 2.474 que sí lo hicieron (Signal Quality pertenece a la creación del LIMIT, no al resultado). **Productor tipado implementado** (`scripts/signal_quality_vector.py`): `SignalQualityVectorV1`/`FactorObservation[T]` como `@dataclass(frozen=True)` — inmutabilidad verificada (mutar lanza `FrozenInstanceError`, no por convención), validación de invariantes en `__post_init__` (enum de Alignment, enum de weekday, direction, consistencia status/value), determinismo verificado (inputs idénticos → vectores estructuralmente iguales); **3.207/3.207 filas del audit CSV construidas sin excepciones** vía `produce_from_row()`. **Arquitectura trazada** (`execution/src/score_store.py::record()` identificado como el único punto de persistencia por-LIMIT existente en producción, escrito en el momento de creación vía `execution/src/bot.py:682`; `panel/app.js` ya tolera campos faltantes) — **decisión explícita de NO cablear el productor a `execution/`/`api/`/`panel/` en esta tarea**, porque ATR-Wilder (dependencia de Momentum/Structure) no existe en ningún archivo de `strategy/` hoy y agregarlo sería un cambio de alcance y riesgo mayor que observabilidad shadow. **No-outcome-leakage:** auditoría grep explícita sobre los 3 scripts nuevos, 0 coincidencias de `pnl/outcome/win/loss/fill_bar/mfe/mae/...` fuera de comentarios que documentan su ausencia. **Behavior invariance:** prueba más fuerte posible — `git diff --stat -- strategy/ execution/ api/ panel/` vacío antes y después, cero archivos de producción tocados. **Tests:** 21/21 checks en `scripts/test_signal_quality_vector.py` (schema, availability, immutability, determinism, no-outcome-leakage) + 6/6 checks internos del script de reconstrucción + 5/5 suites de regresión existentes sin cambios. **Reporte completo (documento maestro, autocontenido):** `reports/BOT-051.3-signal-quality-vector-shadow-observability.md`. Scripts: `scripts/reconstruct_signal_quality_vector_full_universe_xau.py`, `scripts/signal_quality_vector.py`, `scripts/test_signal_quality_vector.py`. Evidence log: `reports/BOT-051.3-SIGNAL-QUALITY-SHADOW-EVIDENCE.log`. Artefacto: `reports/BOT-051.3-signal-quality-vector-shadow-xau.csv` (3.207 filas, sin columnas de outcome).
- **Siguiente paso planificado dentro de BOT-051 (ver `BOT-051.4`):** Live Persistence — extender `execution/src/score_store.py` para exponer el vector en vivo, condicional a evaluar primero si introducir ATR-Wilder en producción es deseable.
- **Dependencias:** BOT-051.2 (`DONE — SIGNAL_QUALITY_VECTOR_V1_FREEZE`, contrato reconstruido en shadow), BOT-024.4 (limitación de re-slice causal standalone cerrada en esta tarea), BOT-047.2.1 (funciones de bloques D1 reusadas sin modificar).

### BOT-051.4 — Signal Quality Live Persistence
- **Categoría:** Scoring / Ejecución / Observabilidad
- **Estado:** DONE
- **Prioridad:** MEDIUM (no bloquea nada aguas abajo; downstream de `BOT-051.3`)
- **Incorporado:** 2026-09-21
- **Versión objetivo:** primera subtarea de esta línea que toca `strategy/`/`execution/`/`api/`/`panel/` (no offline puro como `.1`/`.2`/`.3`).
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.3`. Implementa Signal Quality v1 en el bot productivo — calculado y congelado causalmente al nacer cada LIMIT, asociado al ticket real de MT5, persistido y expuesto por API/panel — **puramente observacional, nunca participa en la decisión de operar.**
- **Resultado (2026-09-21):** **`SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`.** **Consolidación de primitivas** (única fuente de verdad, sin duplicar código): `atr_wilder()` canónica nueva en `strategy/signal_quality.py` (antes no existía ninguna copia dentro de `strategy/`); Alignment reusa `strategy.scoring._closed_blocks()` (ya en producción para Tendencia, BOT-023) para el boundary B0/legacy y una función nueva, `strategy.scoring._closed_blocks_session_anchored()`, para B1/canónico (misma ancla de sesión — `htf_session.bucket_start_utc_seconds` — que ya usa la señal HTF real); ambas alimentan `strategy.scoring._classify_sequence()`, también ya en producción, sin modificarla. Structure reusa `strategy.signal_quality._replay_armado_origin()` (extrae el subconjunto armado/origen del loop de `engine.run_backtest()`, mismo algoritmo verbatim que `BOT-048.1`). El esquema tipado (`SignalQualityVectorV1`/`FactorObservation`) se promovió de `scripts/signal_quality_vector.py` a `strategy/signal_quality.py` — ese script ahora re-exporta desde ahí (una sola fuente de verdad, los 3 scripts dependientes de `BOT-051.1/.2/.3` siguen funcionando sin cambios). **Diseño arquitectónico:** los 4 factores se calculan en vivo con **batch sobre una ventana de mercado recién traída** (`mt5.copy_rates_range`, `SIGNAL_QUALITY_LOOKBACK_DAYS=15`), mismo patrón ya usado en producción por `_score_entry()` — **no** un estado incremental nuevo dentro de `strategy/live_signal.py`, que queda con **diff cero**. **Parity exhaustiva** (`scripts/verify_signal_quality_live_parity_xau.py`, N=3.207, no muestra): `roc_atr_3` vs `BOT-024.2` — 0 discrepancias (error máx. 4,44e-16, ruido de punto flotante); `origin_dist_atr` vs `BOT-048.1` — 0 discrepancias (error máx. 8,88e-16) y 0 casos de autoconsistencia armado/origen fallida; `alignment`/`alignment_status`/`weekday`/`*_status` vs `BOT-051.3` — 3.207/3.207 exact-match (una discrepancia aparente de 27 filas en la primera corrida resultó ser una diferencia de esquema de serialización CSV —`NaN` vs string `"UNAVAILABLE"`—, investigada y corregida, no un bug de cómputo). **Persistencia** (`execution/src/score_store.py`): `record()`/`load_all()` extendidos con `signal_quality` opcional, backward-compatible verificado con test dedicado (10/10 checks: línea legacy sin la clave nueva se lee sin error, línea con solo `signal_quality` también persiste, ticket repetido y línea corrupta se comportan igual que antes). **API:** `/scores` sin cambio de código (ya devolvía `load_all()` verbatim). **Panel:** `panel/app.js::scoreBadge()` extendido (mismo popover, sin romper su comportamiento existente) con vector crudo — sin score, sin colores que impliquen ranking (`panel/style.css`, clases `.sq-*` nuevas, deliberadamente sin reusar `.ok`/`.bad`). **Behavior invariance — CRÍTICO:** prueba A/B dedicada (`execution/src/test_signal_quality_behavior_invariance.py`, MT5 falso inyectado) — el request completo a `mt5.order_send()` es **byte-idéntico** entre "Signal Quality corre normal" y "Signal Quality forzada a lanzar excepción" (11/11 checks); más la prueba estructural más fuerte posible: `strategy/engine.py`/`strategy/live_signal.py` con **diff cero**. **Tests:** 25/25 (`strategy/test_signal_quality.py`) + 10/10 (`execution/src/test_score_store.py`) + 11/11 (behavior invariance) + 21/21 (`scripts/test_signal_quality_vector.py`, sin cambios de comportamiento) + 5/5 suites de regresión existentes sin cambios. **Reporte completo (documento maestro, autocontenido):** `reports/BOT-051.4-signal-quality-live-persistence.md`. Artefactos: `reports/BOT-051.4-signal-quality-live-parity-xau.csv`. Evidence log: `reports/BOT-051.4-SIGNAL-QUALITY-LIVE-EVIDENCE.log`.
- **Siguiente paso planificado dentro de BOT-051 (ver `BOT-051.5`):** Live Accumulation / OOS Bridge — dejar que se acumulen snapshots reales, sin research nuevo hasta tener volumen.
- **Dependencias:** BOT-051.3 (`DONE — SHADOW_OBSERVABILITY_READY`, productor tipado y arquitectura ya trazada).
...
- **Prioridad:** MEDIUM
- **Incorporado:** 2026-09-21
- **Versión objetivo:** primera subtarea de esta línea que agrega infraestructura nueva a `execution/src/` y `api/` mas alla de lo que `BOT-051.4` ya habia tocado.
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.4`. Construye el puente entre Signal Quality ya observable en producción y las validaciones OOS pendientes (`BOT-024.3`/`BOT-047.3`/`BOT-048.3`/`BOT-050.3`) — captura una vez al nacer la LIMIT, observa el resultado despues por separado, nunca reescribe el snapshot original.
- **Resultado (2026-09-22):** **`SIGNAL_QUALITY_OOS_BRIDGE_READY`**, evidencia `ACCUMULATING`. **Hallazgo critico de pre-flight:** se encontraron 5 líneas de contaminación (tickets sintéticos `500000`/`500001`) mezcladas con datos reales en `execution/data/scores/XAUUSDc_900001.jsonl`, causadas por `execution/src/test_signal_quality_behavior_invariance.py` (`BOT-051.4`), que nunca había aislado `score_store.DATA_DIR` a un directorio temporal — corregido (causa raíz + limpieza con backup, verificado con checksum). Tras la limpieza se confirmaron **4 snapshots de Signal Quality genuinos** del 2026-09-22 (posteriores al deploy de `BOT-051.4`) — el bot en vivo del usuario corrió el código nuevo contra la cuenta MT5 **real** (Exness, no demo) y produjo Signal Quality correctamente. **Identidad/linkage** (`order_ticket == position_id == deal.position_id`) verificada con datos reales, no solo argumentada — consultas read-only contra la cuenta confirmaron la equivalencia en los 3 tickets que llenaron. **Hallazgo real no anticipado:** `history_orders_get()` no retiene `sl`/`tp`/`magic` en el registro de la orden ya llenada para esta cuenta/broker (Exness) — `realized_r` queda `UNAVAILABLE` para los 3 trades cerrados reales (definición histórica de R localizada y reutilizada verbatim vía `strategy.costs.BrokerCosts.price_to_usd()`, sin inventar una nueva; `pnl_net` sí se conserva, coincide exacto con la fórmula ya usada por `panel/app.js`). **Reconciliación** (`execution/src/signal_quality_reconciliation.py`, 100% read-only respecto a MT5): 1 `CANCELED`, 3 `FILLED` cerrados (2 `SL`, 1 `TP`) — idempotencia verificada dos veces (fixtures + cuenta real: segunda corrida sin cambios → 0 escrituras nuevas). **Bug real encontrado y corregido durante esa verificación:** la comparación de idempotencia incluía un timestamp de bookkeeping (`reconciled_at_utc`, siempre distinto entre corridas), produciendo idempotencia solo aparente — corregido para excluirlo explícitamente. **Persistencia separada** (`execution/src/outcome_store.py`, nuevo, JSONL propio) — nunca mezclada con `score_store`/`SignalQualityVectorV1` (que sigue `frozen=True`, sin tocar). **Structure availability telemetry** (sección prioritaria del enunciado): razones categorizadas (`warmup`/`alignment_history`/`structure_replay_mismatch`/`market_history_fetch_error`/`unexpected_exception`) agregadas a `strategy/signal_quality.py`/`execution/src/bot.py`, persistidas por separado (`signal_quality_diagnostics`) — sin aumentar el lookback de 15 días arbitrariamente. **Dataset OOS determinista** (`execution/src/signal_quality_oos_dataset.py`, usado tanto por `scripts/build_signal_quality_oos_dataset.py` como por la API): usa los snapshots de `t0` tal cual, nunca los recalcula; separa explícitamente features de "evaluation only" (`pnl_net`/`realized_r`/`close_reason`). **Readiness por factor** (`NO_DATA`/`ACCUMULATING`/`READY_FOR_OOS_ANALYSIS`, nunca `PASS`/`FAIL`/`OOS_VALIDATED`) con criterios no arbitrarios atados a cada hipótesis congelada (cobertura direccional para Momentum/Structure, ≥3 estados sustantivos para Alignment, ≥2 días de semana para Context) — con la evidencia real actual (N=3 cerrados, todos LONG, todos `NEUTRAL`, todos martes) los 4 factores quedan `ACCUMULATING`, correctamente ninguno `READY`. **API** (`GET /signal-quality/oos-status`) y **panel** (tarjeta "Signal Quality — Acumulación OOS", sin colores de calidad/score/gate) verificados **visualmente en navegador contra la cuenta real** (bot permaneció `DETENIDO` durante toda la verificación — cero riesgo de operar), incluyendo el popover de Calificación con la nueva sección Signal Quality mostrando datos reales exactos. **Failure isolation** probada explícitamente (una excepción del bridge no propaga, el bot sigue operando). **Tests:** 26/26 (`execution/src/test_signal_quality_oos_bridge.py`, nuevo) + 11/11 (`test_signal_quality_behavior_invariance.py`, corregido) + 10/10 (`test_score_store.py`) + 25/25 (`strategy/test_signal_quality.py`) + 21/21 (`scripts/test_signal_quality_vector.py`) + 9 suites de regresión existentes sin cambios. `strategy/engine.py`/`strategy/live_signal.py` con diff cero. **Reporte completo (documento maestro, autocontenido):** `reports/BOT-051.5-signal-quality-live-accumulation-oos-bridge.md`. Artefactos: `reports/BOT-051.5-signal-quality-oos-dataset.csv`, `reports/BOT-051.5-signal-quality-oos-coverage.csv`. Evidence log: `reports/BOT-051.5-SIGNAL-QUALITY-OOS-BRIDGE-EVIDENCE.log`.
- **Siguiente paso planificado dentro de BOT-051 (ver `BOT-051.6`):** Production UI / Observability UX — hacer que Signal Quality, que ya existe en producción desde `BOT-051.4`, sea claro, visible y útil para observación humana en el panel (desde la creación del LIMIT, no solo al cerrar el trade). La revisión de OOS Readiness (renumerada `BOT-051.7`) sigue en paralelo, sin bloquear ni ser bloqueada por este trabajo; la falta temporal de N suficiente no detiene ingeniería/UI/observabilidad/documentación.
- **Dependencias:** BOT-051.4 (`DONE — SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`).

### BOT-051.6 — Signal Quality Production UI / Observability UX
- **Categoría:** Scoring / Observabilidad / UI
- **Estado:** DONE
- **Prioridad:** HIGH
- **Incorporado:** 2026-09-22
- **Versión objetivo:** primera subtarea de esta línea que toca `panel/` sin agregar infraestructura backend nueva (solo un campo aditivo en `score_store.load_all()`, sin endpoints nuevos).
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.5`. **Decisión de planificación (2026-09-22): no se detiene el desarrollo esperando volumen OOS** — la evidencia real (`BOT-051.5`/`BOT-051.7`) sigue acumulándose en paralelo mientras el bot opera; el siguiente paso activo es hacer que Signal Quality, que ya se calcula y persiste para cada LIMIT real desde `BOT-051.4`, sea claro, visible y útil para observación humana en el panel — **sin convertirlo todavía en una decisión automática de trading.** Este ítem **no vuelve a implementar el cálculo** (ya congelado/en producción vía `BOT-051.1`–`.5`) — es diseño e implementación de su presentación productiva. Debe contemplar como mínimo: mostrar Signal Quality desde la creación de la LIMIT, no solo al terminar el trade; hacer visible el vector vigente de Signal Quality; mostrar claramente Momentum, Alignment, Structure y Context; respetar `AVAILABLE`/`UNAVAILABLE` y sus razones de indisponibilidad (telemetría ya agregada en `BOT-051.5`); separar inequívocamente información ex ante/`t0` de outcomes ex post — **nunca mezclar `OutcomeObservation` (`execution/src/outcome_store.py`) con el snapshot inmutable de `t0`**; mejorar legibilidad/UX de la superficie actual (`panel/app.js::scoreBadge()`, `BOT-051.4`); analizar primero la ubicación adecuada dentro del panel; conservar trazabilidad con `/scores` y la infraestructura ya construida por `BOT-051.4`/`BOT-051.5`. Permite cambios de `panel/` y, si son necesarios para observabilidad, `api/` — pero **sin cambiar comportamiento de trading** (misma disciplina de behavior invariance que `BOT-051.4`/`BOT-051.5`: `strategy/engine.py`/`strategy/live_signal.py` deben quedar con diff cero, ninguna entrada/salida/SL/TP/sizing/RR/HTF se modifica).
  - **Guardrails explícitos:** NO score 0–100, NO pesos, NO thresholds arbitrarios, NO bins arbitrarios, NO gate de ejecución, NO cambios de entradas/salidas, NO cambios de SL/TP, NO cambios de sizing, NO cambios de RR, NO cambios de HTF, NO reoptimización de factores, NO outcome leakage.
  - **Contratos congelados (no reabrir ningún freeze):** Momentum → `roc_atr_3` (`BOT-024.4`, `FREEZE_READY`). Alignment → Structural Alignment Consensus (`BOT-047.2.3`, `FREEZE_READY`). Structure → `origin_dist_atr` (`BOT-048.2`, `ORIGIN_ONLY_FREEZE`). Context → contrato temporal/`weekday` congelado (`BOT-050.2`, `TEMPORAL_ONLY_FREEZE`). Economics → `EXCLUDED_ABSORBED` (`BOT-049.2`, sin representación que mostrar).
  - **Dirección futura de producto (conceptual, no se implementa acá):** la UI puede prepararse conceptualmente para una representación futura tipo `SIGNAL QUALITY: 78/100`, pero `BOT-051.6` **no calcula ni muestra un score ficticio 0–100** — ese paso sigue sin evidencia/definición (ver `BOT-024`/`BOT-051` principal).
  - **Resultado (2026-09-22):** **`SIGNAL_QUALITY_PRODUCTION_UI_READY`**. **Gap real encontrado y cerrado:** Signal Quality solo era visible en el panel DESPUÉS de que el trade cerraba (tabla "Últimas 20 operaciones") — una LIMIT pendiente o una posición abierta no mostraban nada. `renderPositionsOrders()` (tarjeta "Posiciones y pendientes") ahora lista cada posición/pendiente individualmente con el MISMO ícono/popover ya existente (`scoreBadge()`, reutilizado tal cual — sin superficie redundante), verificado con una posición **real** abierta (#351122611) contra la cuenta MT5 real conectada. **Segundo gap cerrado:** `execution/src/score_store.py::load_all()` (fuente de `GET /scores`) nunca exponía `signal_quality_diagnostics` (la razón de `UNAVAILABLE`, agregada en `BOT-051.5`) — el panel no podía cumplir el contrato de la sección 6 del enunciado ("Reason: <razón real>"). Corregido de forma aditiva/backward-compatible; `panel/app.js::signalQualityFactor()` ahora la muestra. **Separación t0/outcome (sección 9, "crítica"):** título explícito "Signal Quality — al crear la LIMIT (t0)" agregado a la caja existente; probado además a nivel de datos que el snapshot persistido queda byte-idéntico tras reconciliar 3 outcomes distintos (`PENDING`→`FILLED`→`TP`) sobre el mismo ticket. **Bug real encontrado y corregido durante la validación visual:** `.score-pop` usaba `right:0` (válido en su único uso previo, última columna de una tabla ancha) pero se salía de pantalla en el nuevo uso (tarjeta angosta) — corregido centrando el popover sobre el ícono con un clamp de ancho, reverificado en 3 anchos de viewport (534px/1400px/375px). **Decisiones de alcance documentadas (no implementadas a propósito):** sin endpoint nuevo para `OutcomeObservation` por ticket (el contrato actual ya resuelve la separación visual sin ampliar superficie backend); sin badge en `panel/history.html` (vista de archivo histórico simplificada, no la superficie primaria); sin browser de LIMITs `CANCELED` (característica preexistente de todo el panel, fuera de "no rediseñar todo el panel" — siguen siendo observables vía el dataset OOS de `BOT-051.5`). **Tests:** 3 nuevos en `test_score_store.py` (signal_quality_diagnostics round-trip) + 14 nuevos en `test_signal_quality_production_ui.py` (razón de UNAVAILABLE, NEUTRAL≠UNAVAILABLE, inmutabilidad t0 frente a outcomes, CANCELED conserva su snapshot, sin leakage de campos de outcome) + regresión completa existente (26+11+21+25 + kill_switch/mt5_validation) sin cambios, todo exit code 0. `strategy/engine.py`/`strategy/live_signal.py` con diff cero. **Validación visual** contra la cuenta MT5 real conectada (Exness-MT5Real22), bot `DETENIDO` durante toda la sesión, cero órdenes reales enviadas. **Reporte completo:** `reports/BOT-051.6-signal-quality-production-ui-observability-ux.md`.
  - **Siguiente paso planificado dentro de BOT-051 (ver `BOT-051.7`):** OOS Readiness Review — sigue `WAITING FOR OOS READINESS`, evidencia real `ACCUMULATING` (N=3 cerrados evaluables, insuficiente para cualquier criterio ya congelado). No se fuerza ni se estima fecha artificial. Sin más trabajo de ingeniería/UI/observabilidad pendiente y no bloqueado en la línea `BOT-051` en este momento.
- **Dependencias:** BOT-051.4 (`DONE — SIGNAL_QUALITY_LIVE_OBSERVABILITY_READY`, cálculo/persistencia/API/popover base ya existen), BOT-051.5 (`DONE — SIGNAL_QUALITY_OOS_BRIDGE_READY`, telemetría de disponibilidad por factor a reusar).

### BOT-051.6.1 — Signal Quality Production UI Cleanup
- **Categoría:** Scoring / Observabilidad / UI — hotfix de presentación
- **Estado:** DONE
- **Prioridad:** HIGH
- **Incorporado:** 2026-09-22
- **Versión objetivo:** hotfix directo sobre `BOT-051.6` (mismo día), sin infraestructura nueva.
- **Descripción:** Hotfix posterior a `BOT-051.6`. La validación visual posterior mostró una deuda UX: el popover productivo de Signal Quality (`panel/app.js::scoreBadge()`) seguía mostrando ARRIBA la calificación legacy (Divergencia/Tendencia/CVP/Nodo/Total, `strategy/scoring.py`) y DEBAJO Signal Quality v1 — dos modelos conceptuales simultáneos en la misma superficie, con un "Total" (-1/0/+1) que ya no representa el contrato vigente. Además la tarjeta "Signal Quality — Acumulación OOS" ocupaba solo 3 de 4 columnas del grid (`wide3`), dejando espacio vacío a la derecha en desktop. Esta tarea **no borra datos legacy** (verificado antes de tocar nada: `score`/Divergencia/Tendencia/CVP/Nodo se siguen calculando y persistiendo exactamente igual en `strategy/scoring.py`/`execution/src/score_store.py`/`execution/src/bot.py`, `/scores` los sigue devolviendo intactos) — es un cambio puramente de presentación: la calificación legacy sale del popover productivo, no del modelo de datos.
  - **Resultado (2026-09-22):** **`SIGNAL_QUALITY_PRODUCTION_UI_CLEANUP_READY`**. `scoreBadge()` reescrita: el popover queda EXCLUSIVAMENTE con Signal Quality v1 (Momentum/Alignment/Structure/Context/Direction, título "al crear la LIMIT (t0)" preservado) — verificado leyendo `innerText` del DOM real contra una operación genuina (#351122611). El ícono deja de mostrar el total legacy (`-1/0/+1`) y queda neutral (sin colores/valores); un ticket sin `signal_quality` ahora no muestra ningún ícono (antes mostraba el legacy). Columna de la tabla renombrada de "Calificación" a "Signal Quality". `fmtSigned()` eliminada por quedar sin ningún llamador (verificado con grep antes de borrar), junto con el CSS muerto asociado (`.score-icon.ok/.bad`, `.score-row`, `.score-total`). **Tarjeta OOS pasa a full width** (`wide3` → `full`). **Bug real preexistente encontrado y corregido durante la validación visual** (no introducido por `BOT-051.6`, afectaba TODAS las tarjetas `.card.full` ya existentes): en el breakpoint intermedio (`@media max-width:850px`, grid de 2 columnas explícitas) `.wide3` (sin clamp ahí, a diferencia del breakpoint de 520px que sí lo tiene) forzaba una 3ra columna implícita que `grid-column:1/-1` no alcanza (`-1` referencia la última línea del grid EXPLÍCITO) — corregido clampeando `.wide3` a `span 2` en ese breakpoint, mismo criterio ya usado a 520px; efecto colateral positivo: corrige el mismo bug para "Curva de capital" y "Últimas 20 operaciones" (`.full` preexistentes). Verificado sin overflow en 1400px/~534px/~375px (`scrollWidth === innerWidth` en los 3). AVAILABLE/UNAVAILABLE+razón y `NEUTRAL != UNAVAILABLE` preservados sin cambios (verificado con fixture en memoria, mismo mecanismo que `BOT-051.6` — 0 casos `UNAVAILABLE` genuinos en vivo todavía). **Tests:** 18 nuevos en `panel/test_signal_quality_popover_cleanup.py` (guard de regresión a nivel de código fuente sobre `app.js` — sin JS runtime en este proyecto) + regresión completa existente (26+11+15+14+25+31+21 + kill_switch/mt5_validation) sin cambios, todo exit code 0. `strategy/engine.py`/`strategy/live_signal.py`/`execution/src/bot.py` con diff cero. **Reporte completo:** `reports/BOT-051.6.1-signal-quality-production-ui-cleanup.md`.
- **Dependencias:** BOT-051.6 (`DONE — SIGNAL_QUALITY_PRODUCTION_UI_READY`, componente/superficie que este hotfix limpia).

### BOT-051.6.2 — Signal Quality UI / Lifecycle Observability Audit
- **Categoría:** Scoring / Observabilidad — auditoría read-only
- **Estado:** DONE
- **Prioridad:** HIGH
- **Incorporado:** 2026-09-23
- **Versión objetivo:** `AUDIT / READ-ONLY INVESTIGATION` — sin código, sin BACKLOG (en su momento), sin commit/push, mandato explícito de la tarea.
- **Descripción:** El usuario reportó que una posición real (`#351931195`) mostraba el P&L flotante en "Posiciones y pendientes" pero **sin** el badge de Signal Quality al lado, y que la misma operación, ya cerrada, tampoco mostraba Signal Quality en "Últimas 20 operaciones" — mientras otra operación (`#351437683`) sí lo mostraba. Investigación read-only exhaustiva (código real, `score_store`/`outcome_store` en disco, MT5 read-only vía `execution/src/signal_quality_reconciliation.py::reconcile_ticket()`, y el log real del bot en vivo obtenido vía `GET /events` contra el proceso en ejecución) para determinar la causa antes de tocar código.
- **Resultado (2026-09-23):** **`PERSISTENCE_FAILURE_IDENTIFIED`**. Descartado explícitamente: bug de UI/lookup (`scoreBadge()` funciona exactamente como está documentado — `if (!signal_quality) return ""`), bug de identidad/linkage MT5 (topología idéntica entre ambos tickets, `order_ticket==position_id` reconfirmado con datos reales), y el flotante NUNCA "reemplazó" a Signal Quality — son dos expresiones de template independientes (`p.profit` y `scoreBadge(scores, p.ticket)`) que solo coincidían numéricamente por haber una sola posición abierta. **Root cause real:** `strategy/signal_quality.py::_replay_armado_origin()` podía devolver `numpy.bool_` (no `bool` nativo) para `senal_venta_at_last_bar`/`senal_compra_at_last_bar`, filtrado vía la semántica de corto-circuito de `and`/`or` de Python hasta `diagnostics["structure_replay_matches_signal"]` — `json.dumps()` **siempre** falla sobre ese tipo (confirmado empíricamente), rompiendo `score_store.record()` **después** de `order_send()` (trading no afectado) pero **antes** de escribir la línea (sin rastro, ni una línea corrupta). El log real del bot (proceso corriendo desde `2026-09-23T13:23:55Z`) mostró el error EXACTO, dos veces, para las dos únicas señales reales desde ese reinicio — incluyendo una posición que seguía **abierta en el momento de la auditoría** (`#352047367`) con el mismo problema, confirmando que el bug estaba **activo en producción**, no era un incidente aislado. `BOT-051.6.1` **no causó** el problema (código de escritura sin cambios desde `BOT-051.5`) — sí **expuso** un hueco de persistencia que antes era invisible (Signal Quality solo se mostraba en cerradas, y el popover legacy rellenaba con la calificación aunque `signal_quality` faltara — aunque en este caso puntual, ni siquiera el fallback legacy habría salvado el ticket, porque la excepción interrumpía TODO `record()`, `score` incluido). Fix propuesto (no implementado en esta tarea, ver `BOT-051.6.3`): `bool(...)` explícito en el productor. **Reporte completo:** `reports/BOT-051.6.2-signal-quality-ui-lifecycle-audit.md`.
- **Dependencias:** BOT-051.6/BOT-051.6.1 (`DONE`, la superficie auditada).

### BOT-051.6.3 — Signal Quality Persistence Serialization Hotfix
- **Categoría:** Scoring / Observabilidad — hotfix de persistencia/confiabilidad
- **Estado:** DONE
- **Prioridad:** HIGH
- **Incorporado:** 2026-09-23
- **Versión objetivo:** hotfix directo derivado de `BOT-051.6.2`, mismo día.
- **Descripción:** Implementa el fix identificado por la auditoría `BOT-051.6.2` (`numpy.bool_` sin castear en `strategy/signal_quality.py::_replay_armado_origin()`, rompía `json.dumps()` en `score_store.record()` para toda señal real donde Structure replay confirmara la señal — el caso normal, no un edge case). Prioridad explícita: 1) corregir el tipo en el productor, 2) tests con arrays NumPy reales (no fixtures ya saneados) que cubran específicamente `replay_matches == True`, 3) defensa en profundidad opcional en persistencia.
- **Resultado (2026-09-23):** **`SIGNAL_QUALITY_PERSISTENCE_HOTFIX_READY`**. **Fix del productor:** `bool(...)` explícito en 3 puntos de `strategy/signal_quality.py` (`down`/`up` en `_replay_armado_origin()`, `atr_ok`, y `replay_matches` en `compute_signal_quality_at_bar()`) — cero cambios de semántica de Structure/Momentum/Alignment/Context, ningún contrato reabierto. **Tests con arrays NumPy reales:** extendido el fixture ya existente de `_replay_armado_origin()` (`strategy/test_signal_quality.py`) con verificación de tipo nativo + `json.dumps()`; nueva sección que recorre `compute_signal_quality_at_bar()` de punta a punta sobre datos deterministas (seed fija) hasta encontrar un caso real `structure_replay_matches_signal=True` (encontrado en `b=168`) — exactamente el escenario que faltaba cubrir. **Defensa en profundidad evaluada e implementada:** `execution/src/score_store.py::_json_safe()`, normalizador explícito y acotado (solo `numpy.bool_`/`integer`/`floating`, nunca conversión genérica a texto — decisión documentada en el docstring del módulo), probado con un `numpy.bool_`/`numpy.int64` REAL persistido y releído. **Aislamiento de fallas:** `score_store.record()` ahora envuelto en su propio `try/except` en `execution/src/bot.py`, estrictamente posterior a `_place_order()` — un fallo de persistencia/observabilidad ya no puede tumbar el ciclo principal ni dejar la barra sin marcar como procesada (cierra también el riesgo secundario de reprocesamiento de barra que había identificado `BOT-051.6.2`); verificado con un test que fuerza la excepción real de producción y confirma: 1 sola orden, request a `order_send()` byte-idéntico, `_last_processed_time` avanza igual. **Tickets perdidos** (`351931195`/`352047367`) **NO se backfillearon** — quedan como pérdida de evidencia OOS permanente, sin fabricar un snapshot retrospectivo de `t0`. **Tests:** 17 nuevos entre `strategy/test_signal_quality.py`/`execution/src/test_score_store.py`/`execution/src/test_signal_quality_behavior_invariance.py` + regresión completa (10 suites) sin cambios, todo exit code 0. `strategy/engine.py`/`strategy/live_signal.py` con diff cero. **Limitación documentada:** el proceso en vivo del usuario (corriendo desde antes de este fix) no recarga código en caliente — el fix requiere que el usuario reinicie el bot para tomar efecto; no se reinició como parte de esta tarea. **Reporte completo:** `reports/BOT-051.6.3-signal-quality-persistence-serialization-hotfix.md`.
- **Dependencias:** BOT-051.6.2 (`DONE — PERSISTENCE_FAILURE_IDENTIFIED`, root cause identificado).

### BOT-051.6.4 — Restore Signal Quality in Positions & Pending UI
- **Categoría:** Scoring / Observabilidad / UI — restauración de presentación
- **Estado:** DONE
- **Prioridad:** MEDIUM
- **Incorporado:** 2026-09-23
- **Versión objetivo:** hotfix de presentación directo, mismo día.
- **Descripción:** En "Posiciones y pendientes" el P&L flotante por posición (`p.profit`) se mostraba en el extremo derecho de cada fila, duplicando el mismo dato ya presente en la tarjeta dedicada `FLOTANTE`. Esta tarea elimina esa duplicación y devuelve el extremo derecho de cada fila a mostrar exclusivamente Signal Quality (reutilizando `scoreBadge()` tal cual, sin una segunda implementación), con un placeholder neutral ("SQ no disponible") para el caso de un ticket sin snapshot `t0` genuino — sin fabricar ni reconstruir nada.
- **Resultado (2026-09-23):** **`SIGNAL_QUALITY_LIFECYCLE_UI_RESTORED`**. `p.profit` eliminado del template de filas de `renderPositionsOrders()`; nueva `scoreBadgeOrUnavailable()` en `panel/app.js` (delega 100% en `scoreBadge()`, agrega solo el placeholder neutral para el caso sin dato). **Auditoría previa a editar (sección 9 del enunciado):** investigado con `git log --follow`/`git log -p` si algún commit había "reemplazado" el badge por el P&L — **no existe tal commit**: `renderPositionsOrders()` fue creada en `BOT-051.6` con ambos elementos coexistiendo desde el primer día en la misma fila; la percepción de "reemplazo" venía de que 2 tickets reales (`351931195`/`352047367`) nunca tuvieron `signal_quality` persistida (bug de `BOT-051.6.2`, corregido en `BOT-051.6.3`), dejando solo el P&L visible al lado de un espacio vacío. Registrado explícitamente para no repetir la confusión. Verificado con datos reales contra la cuenta MT5 conectada (posición real `#352047367`, bot `DETENIDO`) en 1400px/~534px/~375px sin overflow. `FLOTANTE` intacto (misma sumatoria, mismo elemento). `renderTradesTable()` ("Últimas 20 operaciones") sin cambios. **Tests:** 15 nuevos en `panel/test_signal_quality_popover_cleanup.py` (33 checks totales) + regresión completa (6 suites pedidas) sin cambios, todo exit code 0. `strategy/engine.py`/`strategy/live_signal.py`/`execution/src/bot.py` con diff cero (todo el cambio vive en `panel/`). **Observación (no acción de esta tarea):** durante la validación visual, `Momentum`/`Context` cruzaron orgánicamente a `READY_FOR_OOS_ANALYSIS` — `BOT-051.7` se deja intencionalmente sin abrir. **Reporte completo:** `reports/BOT-051.6.4-restore-signal-quality-positions-pending-ui.md`.
- **Dependencias:** BOT-051.6 (`DONE`, componente `scoreBadge()` reusado), BOT-051.6.3 (`DONE`, corrige la causa real de por qué algunos tickets no tenían snapshot).

### BOT-051.7 — Signal Quality OOS Readiness Review
- **Categoría:** Scoring / Investigación / Validación
- **Estado:** TODO — WAITING FOR OOS READINESS
- **Prioridad:** LOW (depende de volumen/tiempo real, no de trabajo pendiente)
- **Incorporado:** 2026-09-22
- **Versión objetivo:** a definir
- **Descripción:** Subtarea de `BOT-051`, sucesora de `BOT-051.5`. **Renumerada desde `BOT-051.6`** (2026-09-22) al reutilizar ese ID para `Signal Quality Production UI / Observability UX` — decisión de planificación explícita: no se detiene el desarrollo esperando volumen OOS, la acumulación real sigue en paralelo mientras el bot opera. Cuando `execution/src/signal_quality_oos_dataset.py::readiness()` reporte `READY_FOR_OOS_ANALYSIS` para algún factor (criterios ya codificados y documentados en `BOT-051.5` — cobertura direccional/de estados/de días, nunca "N≥50"), revisar manualmente si corresponde avanzar la validación OOS correspondiente (`BOT-024.3`/`BOT-047.3`/`BOT-048.3`/`BOT-050.3`) usando el dataset ya construido por el bridge. **No** reabre ninguna definición congelada, **no** reoptimiza, **no** hace feature fishing (misma disciplina que el resto de la línea experimental). **No** se estiman fechas artificiales ni se relajan los criterios ya codificados para acelerar el readiness. Mientras tanto, la acumulación sigue en paralelo — este ticket no bloquea ni se bloquea por otro trabajo del proyecto (en particular, no bloquea `BOT-051.6`).
- **Dependencias:** BOT-051.5 (`DONE — SIGNAL_QUALITY_OOS_BRIDGE_READY`), volumen real suficiente (evidencia `ACCUMULATING` → algún factor `READY_FOR_OOS_ANALYSIS`).

### BOT-052 — Confluence Reader Evaluation
- **Categoría:** Scoring / Investigación
- **Estado:** IN PROGRESS / RESEARCH
- **Prioridad:** MEDIUM
- **Incorporado:** 2026-09-24
- **Versión objetivo:** a definir (solo si la evidencia OOS lo justifica)
- **Descripción:** Feature padre. Hipótesis de negocio: el indicador externo de TradingView **"Lector de confluencias"** (checks `D✓` = alineación con la apertura diaria, `HCH✓` = patrón hombro-cabeza-hombro sobre las resistencias/soportes del indicador original, más EMAs 50/200 M5 y 200 M15 dibujadas como contexto) podría identificar entradas de mayor calidad para la misma estrategia. Anécdotas de la comunidad y una observación manual reciente (~2 SL sin check entre las últimas ~5 entradas) son **motivación para testear, no evidencia**. Toda integración futura requiere evidencia histórica causal + OOS; ningún sub-ítem toca entradas, gates, Signal Quality ni ejecución salvo que una tarea posterior lo justifique explícitamente.
- **Sub-ítems:** `BOT-052.1` (Historical Discovery, `DONE`), `BOT-052.2` (HCH Definition Freeze + Live Shadow Observability, `DONE`), `BOT-052.3` (HCH OOS Validation + Signal Quality Interaction Analysis, `TODO / BLOCKED_ON_OOS`).
...
- **Incorporado:** 2026-09-24
- **Versión objetivo:** N/A — investigación offline
- **Descripción:** Reconstruct and evaluate the TradingView Lector de confluencias (Daily Open, HCH and EMA context) causally against the canonical XAUUSD historical universes before any production integration. `OFFLINE / DISCOVERY / NO PRODUCTION CHANGES / NO SCORE / NO GATE / NO WEIGHTS`.
- **Resultado (2026-09-24):** dataset `backtests/data/XAUUSDc_M5_latest.parquet` + Config A (sin cambios); Universo A = **3.207** LIMITs, Universo B = **2.474** trades (verificados contra `BOT-050.1`/`BOT-051.3`, sin discrepancias). Pine fuente copiado verbatim a `basecode_tradingview/Lector de confluencias.pine`; sus 4 inputs externos se mapearon a la réplica `usarCausal=true` del indicador original (3.207/3.207 LIMITs son flechas de la réplica, 10/10 entradas en vivo recientes también). **Semántica exacta:** `D✓` = `close` de la barra de la flecha vs `open` de la vela D en curso (estricto); `HCH✓` = flecha mientras el nivel ploteado sigue igual al hombro derecho de un HCH formado sobre los últimos 3 cambios de nivel (consumido tras usarse); las EMAs son **solo visuales** (no participan de ningún check). Hallazgos de fuente: la tolerancia de hombros `1.25` es **matemáticamente vacía** (nunca filtra con cabeza estrictamente mayor), y con niveles causales el HCH ocurre solo tras un reset de bloque HTF (100% de 1.256 formaciones) — no es el HCH clásico. **Causalidad:** 25/25 tests deterministas PASS (prefijo, perturbación futura, escaneo estático, semántica M15 `[1]+lookahead_on`, determinismo) + auditoría de 60 eventos en transiciones de día/M15/bloque/HCH. **Boundary diario:** primario 22:00 UTC (ancla TV medida, B1); 18:00 NY con DST resulta idéntico al 100% (la pausa diaria del oro cae justo en la hora de diferencia); 00:00 UTC (MT5) cambia 17% de los estados (543 inversiones) sin cambiar el signo del efecto. **Resultados (Universo B, ExpR):** `D✓` −0.069 vs −0.038 (Δ −0.031, IC95% [−0.121,+0.055]; como filtro PF 0.899→0.753, sacrifica 895 ganadores por 903 pérdidas); `HCH✓` +0.025 vs −0.051 (Δ +0.076, IC [−0.086,+0.236], N=161=6.5%, signo + en 3/3 sub-períodos, 2/2 direcciones, 14/18 meses; Δ estratificado por los 4 factores SQ +0.06…+0.11 → no redundante); `D✓+HCH✓` N=96, Δ +0.013 (D le resta calidad a HCH: `HCH✓` solo +0.112); EMA: ningún estado con IC que excluya 0, E3 (precio vs EMA200 M15) redundante con D1 Alignment (Cramér V=0.42). 0/11 flags con IC95% que excluya 0. **Trades recientes:** la anécdota se reproduce solo en conteo (2 SL sin check el 22-sep bajo TV22), pero otro SL sí tenía `D✓` y 4/5 TP no tenían check — filtrar por `D✓` habría llevado el resultado en vivo de +152.8 a +2.5 USD. **Clasificaciones:** `D` `WEAK_OR_UNSTABLE`, `HCH` `INSUFFICIENT_SAMPLE`, `D+HCH` `WEAK_OR_UNSTABLE`, EMA `WEAK_OR_UNSTABLE` (E3 `REDUNDANT`). Producción: `git diff -- strategy/ execution/ api/ panel/` vacío. Ver `reports/BOT-052.1-confluence-reader-historical-discovery.md`.
- **Decisión sobre BOT-052.2:** `PARTIAL` — se justifica solo para HCH (definición + observación OOS en shadow). `D`, `D+HCH` y EMA no avanzan.
- **Dependencias:** BOT-005 (réplica `usarCausal=true`), BOT-047.2.2/2.3 (boundaries B0/B1, Alignment), BOT-024.4/BOT-048.2/BOT-050.2/BOT-051.3 (factores SQ v1 para redundancia).
...
- **Prioridad:** LOW
- **Incorporado:** 2026-09-24
- **Versión objetivo:** N/A — shadow, sin cambio de comportamiento
- **Descripción:** Subtarea de `BOT-052`, alcance determinado por la evidencia de `BOT-052.1` (solo HCH). Congela la definición exacta de HCH documentada en `BOT-052.1` §3.4 (mapeo de inputs, `mintick`, consumo, tolerancia vacía — preservada literalmente, NO reinterpretada como hombro-cabeza-hombro clásico) en una única implementación canónica y la registra como observación **shadow pura** por LIMIT en vivo, sin participar de ninguna decisión de trading. **Fuera de alcance (verificado, no violado):** gate, peso, score nuevo, threshold de win rate, cambio de entradas/LIMIT/concurrencia/SL/TP/RR/lot size/kill switch, cambio de Momentum/Alignment/Structure/Context, `D`/EMA (no justificados por `BOT-052.1`).
- **Resultado (2026-09-24):** decisión **`HCH_SHADOW_FREEZE`**. Implementación canónica `strategy/hch.py::HCHEngine` (stateful, incremental), consumida por `scripts/confluence_reader.py::emulate_hch()` (research, refactor sin cambio de comportamiento) y `execution/src/bot.py` (producción). Captura en vivo: `hch_state`/`hch_version`/`hch_captured_at`/`hch_consumed_on_signal_bar` + auditoría (`hch_pivot_1/2/3`/`hch_active_level`/`hch_formation_bar`), persistidos en la misma línea que `signal_quality` (`execution/src/score_store.py`), joineable por diseño. **Paridad histórica EXACTA:** Universo A 3.207 LIMITs = 234 HCH + 2.972 NO_HCH + 1 UNAVAILABLE; Universo B 2.474 evaluables = 161 HCH + 2.313 NO_HCH (verificado evento-a-evento contra `reports/BOT-052.1-confluence-reader-events.csv`). Warm-up de reinicio reutiliza la ventana ya existente del motor de señales (480 barras M5), probado suficiente (peor caso observado 276 barras sobre 300 muestras aleatorias del histórico completo) — no se inventó un número nuevo. 10 requisitos de causalidad cubiertos, incluida una regresión especial (remover/mezclar barras futuras no cambia un evento HCH ya etiquetado). **`TRADING_BEHAVIOR_PARITY = EXACT`:** `git diff --stat -- strategy/engine.py strategy/live_signal.py` vacío en todo momento; `execution/src/test_signal_quality_behavior_invariance.py` PASS con HCH activo. Puente OOS (`execution/src/hch_oos_dataset.py` + `scripts/build_hch_oos_dataset.py`) construido y probado, pero **0 observaciones reales al cierre** (el campo `hch` no existía en producción antes de este commit) — la acumulación arranca recién cuando el bot corra con este código. Panel: sección propia en el popover ("HCH — shadow, sin gate"), sin tocar Signal Quality ni sus colores/badges. 25+33+18 checks nuevos (`strategy/test_hch.py`, `execution/src/test_hch_live_shadow.py`, `execution/src/test_hch_oos_dataset.py`) + regresión completa (14 archivos) sin fallos. Ver `reports/BOT-052.2-hch-definition-freeze-live-shadow.md`.
- **Dependencias:** BOT-052.1 (`DONE`).

### BOT-052.3 — HCH OOS Validation + Signal Quality Interaction Analysis
- **Categoría:** Scoring / Investigación / Validación
- **Estado:** TODO / BLOCKED_ON_OOS
- **Prioridad:** LOW
- **Incorporado:** 2026-09-24
- **Versión objetivo:** a definir
- **Descripción:** Sucesora de `BOT-052.2` (`HCH_SHADOW_FREEZE`, `DONE`). Cuando exista volumen genuino de observaciones HCH en vivo posteriores al deploy de `BOT-052.2` (el puente `execution/src/hch_oos_dataset.py`/`scripts/build_hch_oos_dataset.py` ya acumula y reporta N/W/L/T/WR/PF/ExpR/PnL por cohorte HCH/NO_HCH/UNAVAILABLE, 0 filas al cierre de `BOT-052.2`): (1) evaluar si el efecto direccional encontrado por `BOT-052.1` (ExpR HCH +0.025 vs NO_HCH −0.051, `INSUFFICIENT_SAMPLE`, N=161, IC bootstrap cruza cero) se sostiene fuera de muestra, con criterio pre-registrado antes de mirar resultados; (2) usar los factores crudos de Signal Quality que el dataset ya retiene en cada fila (sin recomputar) para un análisis de interacción HCH × Momentum/Alignment/Structure/Context. **No se auto-aprueba nada de aquí desde `BOT-052.2`** — este ticket no se abre a trabajar hasta que haya evidencia genuina. **Fuera de alcance explícito (igual que `BOT-052.2`):** no se busca ni se optimiza ningún umbral de win rate (ej. 60/65 %), no se combina HCH con otros factores para maximizar una métrica, no se toca ningún gate/peso/score de producción sin una decisión posterior explícita.
- **Dependencias:** BOT-052.2 (`DONE`), datos OOS genuinos (misma barrera que BOT-024.3/BOT-047.3/BOT-048.3 — pero contada desde el deploy de `BOT-052.2`, no desde 2026-09-15, porque el campo `hch` no existía en producción antes de esa fecha).
...
- **F. Signal Quality Integration (`BOT-051`) no exige cinco representaciones numéricas obligatorias — actualizado 2026-09-22 tras `BOT-051.5`:** tabla de estado actual — Momentum (`BOT-024.2` `DONE`, `BOT-024.4` `FREEZE_READY` — canónica `roc_atr_3`, RAW; OOS `BOT-024.3` `BLOCKED`), Alignment (`BOT-047.2.3` `FREEZE_READY`, OOS `READY/WAITING`), Structure (`BOT-048.2` `ORIGIN_ONLY_FREEZE`, OOS `PENDING/WAITING`), Economics (`BOT-049.2` `NO_VALID_ECONOMICS_FREEZE` — **excluida del vector, `EXCLUDED_ABSORBED`, no pendiente**), Context (`BOT-050.2` `TEMPORAL_ONLY_FREEZE` — **representación parcial**: solo `weekday`/Viernes, OOS `BLOCKED_WAITING_GENUINE_NEW_DATA`). `BOT-051.2` congeló el esquema, `BOT-051.3` lo hizo observable en shadow, `BOT-051.4` lo llevó a producción, y **`BOT-051.5` construyó el puente hacia OOS genuino** (decisión `SIGNAL_QUALITY_OOS_BRIDGE_READY`, evidencia `ACCUMULATING`): 4 LIMITs reales encontradas y reconciliadas desde el deploy de `BOT-051.4` (3 cerradas evaluables), identidad `order_ticket==position_id` verificada contra la cuenta MT5 real, `OutcomeObservation` separado del snapshot inmutable de `t0`, readiness por factor con criterios no arbitrarios (ninguno `READY_FOR_OOS_ANALYSIS` todavía con N=3) — vector de 4 slots nominales (no 5), los 4 con Definition Freeze propio, ninguno OOS-validado todavía, **puramente observacional, sin participar en ninguna decisión de trading**. La integración futura debe usar únicamente los factores que sobrevivan sus propios contratos/validaciones — sin pesos, score 0–100, `confidence` ni gate definidos todavía (ver `BOT-051`, `BOT-051.1`–`.6`).
